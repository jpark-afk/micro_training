...
/* ---------------------------------------------------------------------*/
static DDS_Boolean
publisher_main(int rti_dds_domain, char *peer, char *udp_intf,
               ...
               int subscribers, const char *command_line_string)
{
    int sleep_time_ms = 2000;
    DDS_Boolean ok = DDS_BOOLEAN_FALSE;
...
main(int argc, char **argv)
{
    ...
    "\t-interface   <interface>     "
    "- UDP interface to be used for communication"
        "\t-reliable                    "
    ...
    else
    {
        ...
        for (i = 1; i < argc; i++)
        {
            ...
            else if (IS_OPTION(argv[i], "-interface"))
            {
                if ((i == (argc - 1)) || *argv[++i] == '-')
                {
                    AppLog_exception("Missing value after -interface\n");
                    return 0;
                }
                udp_intf = argv[i];
            }
        ...
        return publisher_main(rti_dds_domain, peer, udp_intf, min_message_size,
                              max_message_size, is_reliable,
                              measure_individual_latencies, use_keys, num_iter,
                              event_thread_priority, subscribers, command_line);
...
publisher_main_no_args(void)
{
    ...
    return publisher_main(domainId, peer, udp_intf, min_message_size, max_message_size,
                          is_reliable, measure_individual_latencies, use_keys,
                          num_iter, event_thread_priority, subscribers, NULL);
}
