 ...
 * --------------------
 * 15jun2021,tk MICRO-3061/PR.29173
 * - Added OSAPI_PRECONDITION_ALWAYS to TSeq_set_maximum_w_max and
 *   TSeq_copy_w_max.
 * 25mar2021,tk MICRO-2939/PR#28883
 *   - Use a local begin variable in TSeq_map() to prevent optimizing out
 *     begin.
 * 23feb2021,tk  MICRO-2917/PR#28872
 * 06apr2021,tk MICRO-2964/PR.28907
 *  - Only copy if (src != self) in TSeq_copy.
 * 23feb2021,tk MICRO-2917/PR#28872
 *     - TSeq_set_maximum. Moved precondition to the typed API since self is
 *       accessed before REDA_Sequence_set_maximum() is called. Only the
 *       typed API is supported as a public API in CERT.
 * 23feb2021,tk  MICRO-2861/PR#28592
 *     - TSeq_set_maximum. Made self->_element_size = sizeof(T); common code
 *     - TSeq_copy. Moved precondition to the typed API since self is
 *       accessed before REDA_Sequence_copy() is called. Removed assignment
 *       of self->_element_size in typed API since it can be reliably set
 *       in REDA_Sequence_copy().
 * 17feb2021,tk  MICRO-2841/PR#28686
 *     - Added robustness check to TSeq_copy()
 * 20oct2020,tk  MICRO-2537 Set element size in loan_discontiguous and
 *                          set_contiguous_buffer.
...
{
    RTI_INT32 i;
    RTI_INT32 first_el = begin;

    /* first_el is used to prevent the TASKING compiler from optimizing
     * out the begin value which prevents function level testing.
     */
    for (i = first_el; i <= end; i++)
    {
        if (!map(&buffer[i]))
        {
            return RTI_FALSE;
...
TSeq_set_maximum(struct TSeq *self, RTI_INT32 new_max)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (new_max <= 0) ||
                      ...
                      REDA_Sequence_has_ownership((const struct REDA_Sequence*)self),RTI_TRUE);)
#else
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (new_max < 0) ||
                      !REDA_Sequence_has_ownership((const struct REDA_Sequence*)self),
                      return RTI_FALSE,
