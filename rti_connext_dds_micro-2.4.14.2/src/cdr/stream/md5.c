...
static void md5_append(md5_state_t *pms, const md5_byte_t *data, unsigned int nbytes);
...
md5_process(md5_state_t * pms, const md5_byte_t *data /*[64] */ )
{
    ...
    static const int w = 1;
...
#if BYTE_ORDER <= 0             /* little-endian */
        {
             ...
             */
            if (!((data - (const md5_byte_t *)OSAPI_ADDRESS_ZERO) & 3))
            {
                /* This causes alignment increase warning without the cast to
                 * const void* first. However, data is properly aligned (all
                 * buffers are), thus it is safe to realign data.
                 */
...
#  endif
            for (i = 0; i < 16; ++i, xp += 4)
                xbuf[i] = (md5_word_t)xp[0] + (md5_word_t)(xp[1] << 8) + (md5_word_t)(xp[2] << 16) + (md5_word_t)(xp[3] << 24);
        }
#endif
    }
 ...
 *
 * \paramm[in] pms The MD5 processor to initialize
 */
void
...
md5_append(md5_state_t *pms, const md5_byte_t *data, unsigned int nbytes)
{
    ...
    unsigned int offset = (pms->count[0] >> 3U) & 63U;
