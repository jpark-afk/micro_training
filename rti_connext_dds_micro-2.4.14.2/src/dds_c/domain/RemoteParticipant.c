 ...
 * --------------------
 * 13dec2021,tk MICRO-3364/PR.30016
 * - Call NDDS_Discovery_Plugin_assert_remote_participant inside the
 *   critical section in NDDS_DomainParticipant_assert_remote_participant.
 ...
 *   DDS_DataWriter_remove_peer
 *   DDS_DataReader_remove_peer
 *   DDS_DataWriter_add_peer
 *   DDS_DataReader_add_peer
 * 13apr2021,tk MICRO-3028/PR.28960
 * - Removed inclusion of "TopicQos.h" for CERT
 * 06apr2021,tk MICRO-3003/PR.29038
 * - Removed redundant #ifdef RTI_CERT in NDDS_RemoteParticipantRecord_finalize
 * - Removed redundant #ifdef RTI_CERT in NDDS_RemoteParticipant_remove_internal
 * 24feb2021,tk MICRO-2912/PR#28850
 *   - Compare DDS_ENTITYNAME_QOS_NAME_MAX in
 *     NDDS_DomainParticipant_lookup_name(),not DDS_ENTITYNAME_QOS_NAME_MAX - 1.
...
/*ci \brief OMG defined name for the ParticipantBuiltinTopicData Type
 */
const char *const DDS_PARTICIPANT_BUILTIN_TOPIC_TYPE_NAME =
                                        "DDS_ParticipantBuiltinTopicData";

/*ci \brief OMG defined name for the ParticipantBuiltinTopicData Topic
 */                                        
...
NDDS_RemoteParticipant_on_liveliness(struct OSAPI_TimeoutUserData *storage)
{
    ...
    bretval = NDDS_Discovery_Plugin_on_remote_participant_liveliness_expired(
    ...
    if (!bretval)
    {
        DDSC_LOG_DISC_REMOTE_PARTICIPANT_EXPIRED(OSAPI_LOGKIND_WARNING,
                            ...
                            DDS_Boolean *const is_new)
{
    ...
    else
    {
        ...
        if ((dbrc == DB_RETCODE_OK) && (data->participant_name.name[0] != 0) && record->is_static)
        {
    ...
    if ((record == NULL) && (dbrc == DB_RETCODE_NO_DATA))
    {
        ...
        if (dbrc != DB_RETCODE_OK)
        {
            DDSC_LOG_RECORD_CREATE(OSAPI_LOGKIND_ERROR,DDSC_LOG_PARTICIPANT_RECORD,dbrc)
        ...
        if (!DDS_ParticipantBuiltinTopicData_initialize(&record->data))
        {
            ...
            (void)DB_Table_delete_record(participant->remote_participant_table,
                                         (DB_Record_T)record);
            retcode = DDS_RETCODE_OUT_OF_RESOURCES;
            goto done;
        }
        ...
        record->is_static = DDS_BOOLEAN_FALSE;
        if (assign_guid)
        {
            ...
            record->data.key.value[DDS_BUILTIN_TOPIC_KEY_TYPE_NATIVE_LENGTH-1]
                                           = assigned_oid;
            record->is_static = DDS_BOOLEAN_TRUE;
        }
    ...
    {
        DDSC_LOG_RECORD_ERROR(OSAPI_LOGKIND_ERROR,DDSC_LOG_PARTICIPANT_RECORD,dbrc)
        goto done;
    }
    else
    {
        ...
        if (!bretval)
        {
            DDSC_LOG_QOS_CHANGED(OSAPI_LOGKIND_WARNING,DDSC_LOG_PARTICIPANT_QOS)
        }
#endif
    }
    ...
    if ((retcode == DDS_RETCODE_OK) && (participant->disc_plugin != NULL))
    {
        NDDS_Discovery_Plugin_assert_remote_participant(
            participant->disc_plugin,participant,data->participant_name.name);
    }
...
NDDS_RemoteParticipantRecord_finalize(const void *record, void *param)
{
    ...
    if (!DDS_ParticipantBuiltinTopicData_finalize_no_dealloc(&rem_part->data))
    {
        return RTI_FALSE;
    }
...
/* reset_entry is always TRUE for RTI_CERT */
    ...
    {
        if (!DDS_ParticipantBuiltinTopicData_finalize_no_dealloc(&remote_participant->data))
        {
        ...
        if (dbrc != DB_RETCODE_OK)
        {
            DDSC_LOG_RECORD_DELETE(OSAPI_LOGKIND_ERROR,DDSC_LOG_PARTICIPANT_RECORD,dbrc)
            retcode = DDS_RETCODE_ERROR;
            goto done;
        }
    }
