...
/*ci
 * \brief Test if two DDS_ProductVersion structures are equal
 *
 ...
 */
DDS_Boolean
DDS_ProductVersion_is_equal(const struct DDS_ProductVersion *left,
                            const struct DDS_ProductVersion *right)
{
    ...
    return OSAPI_Memory_compare(left, right,sizeof(struct DDS_ProductVersion)) == 0 ?
