 ...
 * - Exclude _state_deleting flag and _wait_ended semaphore for CERT in
 *   WaitSet_wait and WaitSet_new because WaitSet_delete is excluded.
 * 14sep2021,tk MICRO-3267/PR.29652
 * - Remove redundant if-test and done label in
 *   DDS_WaitSetImpl_on_condition_triggered
 * 20jul2021,tk MICRO-3127 Addressed MISRA-C 2021 and SEI CERT C warnings:
 * - Added suppression for dereference and cert_exp34_c_violation
 *   in DDS_WaitSet_get_conditions and DDS_WaitSetImpl_cond_map_wait_postscan
    ...
    *done = RTI_FALSE;
    ...
    if (cref->flag_return_next || cref->flag_pending_trigger)
    {
        ...
        else
        {
            ...
            /* active_conditions has been successfully sized
             */
                        ...
                        DDS_Boolean trigger_value)
{
    ...
    {
        self->_state_blocked = DDS_BOOLEAN_FALSE;
        /* Only a single thread can block on a WaitSet (checked in
         * WaitSet_wait) or updating the WaitSet. Thus, the Coverity
         * [lock_order] event is marked as 'Intentional'.
         */
        /* coverity[lock_order] */
        bretval = OSAPI_Semaphore_give(self->_condition_active);
#if OSAPI_ENABLE_LOG
        if (!bretval)
        {
            OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_ERROR)
...
#endif
    /* The Coverity event [missing_lock] is marked as 'Intentional' since
     * DDS_WaitSet_new function must return before a WaitSet can be used
     * and thus there is not race condition.
     */
                 ...
                 const struct DDS_Duration_t *timeout)
{
    ...
    /* This is the maximum timeout in ms for a wait-set. This is because the
     * duration is converted to ms when taking a semaphore.
     */
    struct DDS_Duration_t max  = { 0x7fffffff/1000 - 1, 0};

    OSAPI_PRECONDITION_ALWAYS(
                  (self == NULL) || (active_conditions == NULL) ||
                  (timeout == NULL) ||
                  (DDS_Duration_compare(timeout,&DDS_DURATION_INFINITE) &&
                   (DDS_Duration_compare(timeout,&max) > 0)),
                   return DDS_RETCODE_BAD_PARAMETER,
    ...
    {
        ms = OSAPI_SEMAPHORE_TIMEOUT_INFINITE;
    }
    else
    {
        ms = DDS_Duration_to_ms(timeout);
    }
    ...
    if (!OSAPI_Mutex_give(self->_state_lock))
    {
        /* self->_state_waiting is accessed outside of a lock in WaitSet_new()
         * where the locks are created. Thus, the Coverity event [missing_lock]
    ...
    if (!OSAPI_Semaphore_take(self->_condition_active,ms,&fc))
    {
        self->_state_waiting = DDS_BOOLEAN_FALSE;
        rc = DDS_RETCODE_ERROR;
        goto done;
                           ...
                           struct DDS_ConditionSeq *attached_conditions)
{
    ...
    while (!REDA_CircularList_node_at_head(&self->_conditions,&cref->_node))
    {
        /* The attached_conditions sequence length has been valided above
         */
        /* coverity[cert_exp34_c_violation] */
        /* coverity[dereference] */
