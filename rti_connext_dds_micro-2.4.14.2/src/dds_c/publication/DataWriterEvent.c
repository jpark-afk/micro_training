 ...
 * - Removed UNUSED_ARG(key) in on_sample_removed
 * 20jul2021,tk MICRO-3131/PR.29255
 * - Removed redundant tests on route_existed and matched in
 *   DDS_DataWriterEvent_on_publication_matched
 * 04apr2021,tk MICRO-2845/PR.28682
 *  - Removed empty blocks in DDS_DataWriterEvent_on_sample_removed()
                                      ...
                                      DDS_Long ack_count)
{
    ...
    if (kind != DDSHST_WRITER_SAMPLE_REMOVED_NORMAL)
    {
        ...
        brc = NETIO_Interface_xmit_remove(datawriter->dw_intf, NULL,sn);
#if OSAPI_ENABLE_LOG
        if (!brc)
        {
    ...
    if (cdr_sample && (cdr_sample->payload != NULL))
    {
        REDA_BufferPool_return_buffer(datawriter->cdr_payloads,cdr_sample->payload);
