 ...
 * --------------------
 * 20jul2021,tk MICRO-3131/PR.29255
 * - Removed redundant tests on route_existed and matched
 *   in DDS_DataReaderEvent_on_subscription_matched
 * 04apr2021,tk MICRO-2845/PR.28682
 *   - Removed empty blocks in DDS_DataReaderEvent_on_sample_committed()
 ...
 * \param[in] source The peer NETIO interface that changed to alive
 */
void
 ...
 * \param[in] source     The NETIO interface address the loss was detected on
 */
void
                                       ...
                                       struct NETIO_Address *source)
{
    DDS_DataReaderEvent_on_remote_writer_event(
            datareader,source,DDSHST_READEREVENT_KIND_LIVELINESS_LOST,RTI_TRUE);
}
 ...
 *
 * \param[in] datareader  The datareader with a change in instance liveliness
 * \param[in] source      The NETIO interface address that was deleted
...
{
    DDS_DataReaderEvent_on_remote_writer_event(
      datareader,source,DDSHST_READEREVENT_KIND_REMOTE_WRITER_DELETED,is_active);
}
                                        ...
                                        DDS_Long committed_samples)
{
    ...
    brc = NETIO_Interface_post_event(self->rtps_intf,NULL,&evnt);
    IGNORE_RETVAL(brc);
}
