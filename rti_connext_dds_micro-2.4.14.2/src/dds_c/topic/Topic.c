 ...
 * 06apr2021,tk MICRO-2938/PR.28900
 *  - Corrected comment for return value for DDS_Topic_get_object_suffix
...
DDS_TopicImpl_finalize(struct DDS_TopicImpl *topic)
{
    ...
    {
        REDA_String_free(topic->topic_name);
        topic->topic_name = NULL;
    }
#endif
...
#else
    topic->topic_name = REDA_String_dup(topic_name);
#endif
    if (topic->topic_name == NULL)
    {
        goto done;
...
/*ci
 * \brief Calculate the object id suffix for a datareader/datawriter given an
 *        integer
 *
 * \param[in] self        The topic to generate a topic id for
 * \param[in] plugin_data The plugin data, typically passed in from the Qos
 * \param[in] oid         The initial object id, an integer
 ...
 *
 * \return The generated object id on success, 0 on failure
 */
DDS_UnsignedLong
                            ...
                            DDS_Boolean is_writer)
{
     ...
     key_kind = DDS_TypeImpl_get_plugin(DDS_Topic_get_type(self))->get_key_kind(
                         DDS_TypeImpl_get_plugin(DDS_Topic_get_type(self)),
                         plugin_data);
...
DDS_Topic_get_qos(DDS_Topic *self, struct DDS_TopicQos *qos)
{
    ...
    if (DDS_TopicQos_copy(qos, &self->qos) != DDS_RETCODE_OK)
    {
...
DDS_Topic_set_qos(DDS_Topic *self, const struct DDS_TopicQos *qos)
{
    ...
    retcode = DDS_TopicQos_copy(&self->qos, qos);
