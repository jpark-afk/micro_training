                                ...
                                DDS_Boolean *dropped)
{
    ...
    *dropped = RTI_FALSE;
    ...
    if (DDS_DataWriter_lookup_route(dpde->participant_writer,&dst_reader,
        ...
        for (i = 0; i < l; ++i)
        {
            ...
            else
            {
                    ...
                    || info->instance_state == DDS_NOT_ALIVE_NO_WRITERS_INSTANCE_STATE)
                {
                    ...
                    OSAPI_TRACE_DDS("[DPDE] Delete participant",RTI_FALSE)
                    OSAPI_TRACE_GUID("key",&key,RTI_TRUE)

                    DPDE_DiscoveryPlugin_remove_builtin_peers(dpde_plugin,&key);
