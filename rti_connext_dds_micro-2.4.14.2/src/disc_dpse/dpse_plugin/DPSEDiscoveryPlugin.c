 ...
 * - Use OSAPI_PRECONDITION_ALWAYS instead of if() in
 *   DPSE_DiscoveryPluginProperty_initialize
 * - Renamed dst to self in DPSE_DiscoveryPluginProperty_initialize
 * - Return DDS_RETCODE_PRECONDITION_NOT_MET in
 *   DPSE_DiscoveryPluginProperty_initialize
        ...
        struct DDS_ParticipantBuiltinTopicData *const local_participant_data)
{
    ...
    {
        DPSE_LOG_GET_DDS_PROPERTIES(OSAPI_LOGKIND_ERROR)
        goto finally;
    }
        ...
        struct DDS_BuiltinTopicKey_t *const local_participant_key)
{
    ...
    if (dpse_plugin->participant_publisher != NULL)
    {
        ...
        {
            if (!DPSE_DiscoveryPlugin_announce_local_participant_deletion(
                                     discovery_plugin, participant,local_participant_key))
            {
                DPSE_LOG_ANNOUNCE_LOCAL_PARTICIPANT_DELETION(OSAPI_LOGKIND_ERROR)
                goto done;
            }
                ...
                const struct DDS_BuiltinTopicKey_t *const local_datawriter_key)
{
    UNUSED_ARG(discovery_plugin);
    UNUSED_ARG(participant);
    UNUSED_ARG(local_datawriter_key);
                ...
                struct DDS_BuiltinTopicKey_t *const local_datareader_key)
{
    UNUSED_ARG(discovery_plugin);
    UNUSED_ARG(participant);
    UNUSED_ARG(local_datareader_key);
        ...
        DDS_Boolean delete_keys)
{
    ...
    /* Reset with the core */
    if (NDDS_DomainParticipant_reset_remote_participant(participant,key)
                                                             != DDS_RETCODE_OK)
    {
        return DDS_BOOLEAN_FALSE;
    }
...
/*ci
 * \brief Called by the participant when a remote participant has failed to
 *        maintain liveliness.
 *
 * \details
 * Implementation of NDDS_Discovery_Plugin_on_remote_participant_liveliness_expired
        ...
        const char *const participant_name)
{
     ...
     * that all the information is valid and legal.
     */
    OSAPI_Memory_copy(rp->name,participant_name,
                      OSAPI_String_length(participant_name)+1);
