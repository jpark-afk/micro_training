 ...
 * --------------------
 * 13sep2021,tk MICRO-3263 / PR.29651
 * - Copy the address in DPSE_ParticipantBuiltinDataReaderListener_add_route,
 *   do not set it as UDPv4.
 * 16apr2021,tk MICRO-2845/PR.28682
 *  - Removed empty statements (LOG statements ending in ;)
 * 06apr2021,tk MICRO-2975/PR.29001
 * - Replace RTI_TRUE/RTI_FALSE with DDS_BOOLEAN_TRUE/DDS_BOOLEAN_FALSE in
                            ...
                            struct DDS_LocatorSeq const *locator_seq)
{
    ...
    for (i = 0; i < j; i++)
    {
        ...
        dst_address = *((struct NETIO_Address*)((void*)a_locator));
    ...
    do
    {
        ...
        for (k = 0; k < l; ++k)
        {
                ...
                info->instance_state == DDS_NOT_ALIVE_NO_WRITERS_INSTANCE_STATE)
            {
                 ...
                 */
                bretval = DPSE_Plugin_reset_remote_participant(
                            dpse_plugin,participant,&key,DDS_BOOLEAN_FALSE);
#if OSAPI_ENABLE_LOG
                if (!bretval)
                {
