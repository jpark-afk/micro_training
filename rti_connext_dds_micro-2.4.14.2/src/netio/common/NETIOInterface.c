 ...
 * - Removed definition of constant NETIO_DEFAULT_UDP_NAME 
 *   when builtin UDP plugin is disabled
 * 20jul2021,tk MICRO-3127 Addressed MISRA-C 2021 and SEI CERT C warnings:
 * - Added cert_str31_c_violation suppression in NETIO_Interface_Table_name_from_id
                                   ...
                                   char suffix,RTI_INT32 instance)
{
    ...
    for (j = 28; (j >= 0); j -= 4, ++i)
    {
        ...
        /* Since  0 <= d <= 15 it cannot exceed the table of 16 entries
         */
