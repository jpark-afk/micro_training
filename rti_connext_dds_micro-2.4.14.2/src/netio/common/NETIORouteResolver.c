 ...
 * --------------------
 * 06jun2022,tk MICRO-3537/PR.30409
 * - Use NETIO_Address_get_kind instead of ->kind in
 *   NETIO_RouteResolver_compare_kind
 * 27apr2022,jh MICRO-3547
 * - Added NETIO_ROUTE_OPERATION_ADD_PEER to differentiate from add route
 * - Updated NETIO_RouteResolver_route_update
 *   - Also call add_route and delete_route on the lower level interface
 *   - Allow NETIO_ROUTE_OPERATION_ADD to fail
 * - Update NETIO_RouteResolver_add_peer to use NETIO_ROUTE_OPERATION_ADD_PEER
 * 17sep2021,tk MICRO-3276/PR.29664
 * - Return RTI_FALSE in NETIO_RouteResolver_add_peer if no addresses were
 *   added or if there was at least one call to add_route failed. Set output
 *   found_route value on return regardless TRUE/FALSE.
 * 15jun2021,tk MICRO-3069/PR.29115
 * - Removed redundant code in NETIO_RouteResolver_compare_entry
 * 05aug2020,tk MICRO-2364/PR#27563 Moved NETIO_calculate_mask_index() after
 *                                  SOURCE_BEGIN
                                  ...
                                  const DB_Record_T op1, void *op2)
{
    ...
    for (i = 0; i < j; ++i)
    {
         ...
         */
        if ((left->address.value.as_uint32.value[i] & left_mask_value) >
            (right->address.value.as_uint32.value[i] & left_mask_value))
        {
            diff = -1;
            break;
        }
        else if ((left->address.value.as_uint32.value[i] & left_mask_value) <
                 (right->address.value.as_uint32.value[i] & left_mask_value))
        {
            diff = 1;
            break;
                    ...
                    struct NETIO_RouteResolverRecord *r_entry)
{
    ...
    for (i = 0; i < j; ++i)
    {
        ...
        diff = ((r_entry->address.value.as_uint32.value[i] & left_mask_value) -
                (address->value.as_uint32.value[i] & left_mask_value));
