 ...
 * - Updated LOOP_Interface_send() to check for packet->dests 
 *   and return false if it is NULL. 
 * 03may2022,jh MICRO-3547
 * - Removed precondition check for via_intf and via_addr in
 *   LOOP_Interface_add_route and LOOP_Interface_delete_route
 * 10mar2022,jh MICRO-3511
 * - Added missing coverity dereference and cert_exp34_c_violation suppression
 *   in LOOP_Interface_get_route_table
 * 20jul2021,tk MICRO-3127 Addressed MISRA-C 2021 and SEI CERT C warnings:
 * - Added coverity dereference and cert_exp34_c_violation suppression in
                               ...
                               RTI_BOOL *is_invalid)
{
    ...
    *is_invalid = RTI_FALSE;
    ...
    NETIO_Address_set_kind(address_value,NETIO_ADDRESS_KIND_INTRA,
                           NETIO_ADDRESS_FLAG_INTERNAL);
...
/*ci
 * \brief Release a preivously reserved address
 *
 * \details
