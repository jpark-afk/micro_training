...
/*ci \brief Pre-allocated mutex structure to avoid memory allocations during
 * initialization.
 */
RTI_PRIVATE struct OSAPI_Mutex OSAPI_AutosarHeap_fv_MutexMemory;
...
OSAPI_AutosarHeap_is_initialized(void)
{
    ...
    /* Once memory is configured we can create a mutex for memory allocations. 
     * Use preallocated memory to avoid allocations during initialization.
     */
    OSAPI_AutosarHeap_fv_Mutex = OSAPI_AutosarMutex_initialize_mutex(
                                        &OSAPI_AutosarHeap_fv_MutexMemory);

    if (OSAPI_AutosarHeap_fv_Mutex == NULL)
    {
        /* This is safe since only pointers are reset */
        return FALSE;
    }
