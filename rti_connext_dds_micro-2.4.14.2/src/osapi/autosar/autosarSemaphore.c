...
OSAPI_Semaphore_new(void)
{
    ...
    if ((OSAPI_Autosar_fv_CurrentResourceId == 0) &&
        OSAPI_System_gv_PortProperty->use_udp_thread &&
        (semaphore->task_id != OSAPI_System_gv_PortProperty->udp_receive_task_id))
    {
        /* Skip the first resource if use_udp_thread is TRUE as it is
         * reserved for the UDP task. OSAPI_Autosar_fv_CurrentResourceId
         * is incremented here instead of initializing to 1 to keep the
         * test for exceeding semaphore_max_count the same.
         */
        OSAPI_Autosar_fv_CurrentResourceId++;
    }
#endif
...
#if OSAPI_THREAD_SEMAPHORE_ENABLED
    if ((semaphore->task_id == OSAPI_System_gv_PortProperty->udp_receive_task_id)
        && OSAPI_System_gv_PortProperty->use_udp_thread)
    {
        /* The first offset is reserved for the UDP task if enabled. This is
         * necessary because the AUTOSAR configuration needs to allocate
         * specific resources to specific tasks. The UDP task adds exactly
         * one semaphore once and the first offset for first_give_event,
         * first_timeout_event and first_alarm must be configured for the
         * UDP receive task.
         */
        semaphore->id_offset = 0;
    }
    else
#endif
    {
