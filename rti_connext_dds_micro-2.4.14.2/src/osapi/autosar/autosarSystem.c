...
/*ci \brief Maximum number of timers that the Autosar system can create.
 */
...
#define OSAPISYSTEM_TIMER_RESOLUTION_US \
    (OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_NTP_TIME_USEC_PER_MSEC)

/*ci \brief Timer resolution in nanoseconds.
 */
#define OSAPISYSTEM_TIMER_RESOLUTION_NS \
    (OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_NTP_TIME_NSEC_PER_MSEC)

/*ci \brief System timer handler structure.
 */
struct OSAPI_SystemTimerHandler
{
    /*ci \brief Timer handler.
     */
    OSAPI_TimerTickHandlerFunction handler;

    /*ci \brief Param for timer handler.
     */
...
/*ci \brief Autosar system structure.
 */
struct OSAPI_SystemAutosar
{
    /*ci \brief Parent System.
     */
    struct OSAPI_System _parent;

#ifndef RTI_CERT
    /*ci \brief RTI_TRUE if System has been deleted after it was initialized.
     * Needed to synchronize timers with task deleting the system.
     */
    RTI_BOOL is_deleted;
#endif

    /*ci \brief Array with all timer handlers.
     */
    struct OSAPI_SystemTimerHandler timer_handler[OSAPISYSTEM_MAX_TIMERS];

    /*ci \brief Current number of timers created.
     */
    RTI_INT32 timer_count;

    /*ci \brief System lock mutex.
     */
    P2VAR(struct OSAPI_Mutex, AUTOMATIC, SOAD_APPL_DATA) mutex;

    /*ci \brief Current tick seconds.
     */
    RTI_INT32 tick_sec;

    /*ci \brief Current tick nanoseconds.
     */
    RTI_UINT32 tick_nanosec;

    /*ci \brief System tick lock mutex.
     */
 ...
 */
RTI_PRIVATE struct OSAPI_SystemAutosar OSAPI_System_g = OSAPI_SystemAutosar_INITIALIZER;
...
OSAPI_System_gv_System = &OSAPI_System_g._parent;

/*ci \brief Pointer to Autosar System property.
 */
P2VAR(struct OSAPI_PortProperty, AUTOMATIC, SOAD_APPL_DATA)
OSAPI_System_gv_PortProperty = &OSAPI_System_g._parent.property.port_property;
...
OSAPI_SystemAutosar_check_mutex_properties(void)
{
    ...
    {
        if (OSAPI_System_gv_PortProperty->first_resource_id >=
            OSAPI_System_gv_PortProperty->last_resource_id)
        {
            return FALSE;
        }
    }
...
OSAPI_SystemAutosar_timer_callback(void)
{
     ...
     */
    if (OSAPISYSTEM_TIMER_RESOLUTION_NS >= OSAPI_NTP_TIME_NSEC_PER_SEC)
    {
        /* Add seconds in the resolution to make sure
         * the remainder is less than 1 sec
         */
        self->tick_sec = self->tick_sec +
               (OSAPISYSTEM_TIMER_RESOLUTION_NS / OSAPI_NTP_TIME_NSEC_PER_SEC);
...
DeclareTask(OSAPI_SystemAutosar_timer_task);

/*ci \brief Timer task. User shall include this task in the Autosar
 * configuration or function OSAPI_SystemAutosar_timer_callback()
 * can be called from another task.
 */
TASK(OSAPI_SystemAutosar_timer_task)
{
    StatusType status;
    ...
    if (status != E_OK)
    {
        OSAPI_LOG_AUTOSAR_TERMINATETASK_CALL(OSAPI_LOGKIND_ERROR, status);
    }
#else
...
OSAPI_AutosarSystem_get_time(P2VAR(OSAPI_NtpTime, AUTOMATIC, SOAD_APPL_DATA) now)
{
#if 1
    RTI_INT32 sec = 0;
    RTI_UINT32 nanosec = 0;
#else
    /* according to EB documentation: A timestamp is a counter that can never
     * overflow during the expected up-time of the processor.
     */
    os_timestamp_t current_time;
    RTI_UINT32 nanosec;
#endif
    ...
    OSAPI_NtpTime_from_nanosec(now, sec, nanosec);
#else
    OS_GetTimeStamp(&current_time);
    /*void MK_ReadTime(mk_time_t *timeRef);*/
    /*nanosec = OS_TimestampTicksToNs(current_time);*/
    nanosec = MK_TimestampTicksToNs(current_time);
    OSAPI_NtpTime_from_microsec(now, 0, /*nanosec / 1000 */ 1000);
#endif
    ...
    P2VAR(RTI_UINT32, AUTOMATIC, SOAD_APPL_DATA) address)
{
    ...
    for (idx = 0; idx <= OSAPI_System_gv_PortProperty->max_local_addr_id; idx++)
    {
        ...
        {
#if MENTOR_OLD_COMPAT
            if ((((TcpIp_SockAddrType *)&ipVxAddrBuffer[0])->TcpIp_Domain == TCPIP_AF_INET)
#else
            if ((((TcpIp_SockAddrType *)&ipVxAddrBuffer[0])->domain == TCPIP_AF_INET)
#endif
