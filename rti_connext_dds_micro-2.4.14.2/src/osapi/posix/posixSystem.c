...
OSAPI_SystemPosix_run_timers(struct OSAPI_SystemPosix *self)
{
    ...
    for (i = 0; (i < OSAPISYSTEM_MAX_TIMERS); ++i)
    {
        ...
        if (self->timer_handler[i].deleted)
        {
            ...
            /* At this point the tiner may get re-allocated and set the
 ...
 */
RTI_PRIVATE RTI_BOOL
OSAPI_SystemPosix_get_time(OSAPI_NtpTime *now)
{
#if ENABLE_FACE_COMPLIANCE > FACE_COMPLIANCE_LEVEL_NONE
    ...
    OSAPI_NtpTime_from_nanosec(now,
                                (RTI_INT32)current_time.tv_sec,
                                (RTI_UINT32)current_time.tv_usec*1000);
    ...
    OSAPI_NtpTime_from_nanosec(now, current_time.tv_sec, current_time.tv_nsec);
...
OSAPI_System_get_native_interface(struct OSAPI_SystemI *intf)
{
    ...
    intf->start_timer = OSAPI_PosixSystem_start_timer;
#ifndef RTI_CERT
    intf->stop_timer = OSAPI_PosixSystem_stop_timer;
#else
    intf->stop_timer = NULL;
#endif
    intf->get_timer_resolution = OSAPI_PosixSystem_get_timer_resolution;
    intf->get_time = OSAPI_SystemPosix_get_time;
    intf->initialize = OSAPI_SystemPosix_initialize;
#ifndef RTI_CERT
    intf->finalize = OSAPI_SystemPosix_finalize;
#else
    intf->finalize = NULL;
#endif
    intf->generate_uuid = OSAPI_SystemPosix_generate_uuid;
    intf->get_hostname = OSAPI_SystemPosix_get_hostname;
    intf->get_ticktime = OSAPI_PosixSystem_get_ticktime;
}
