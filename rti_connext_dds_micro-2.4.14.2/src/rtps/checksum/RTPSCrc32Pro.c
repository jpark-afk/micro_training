/*
 * FILE: RTPSCrc32Pro.c - Implementation of standard 32bit CRC
 *
 ...
 * 19oct2020,tk MICRO-2575/PR#28172 Renamed RTPS_BuiltinCrc32_checksum_calculate
 *                                  to RTPS_BuiltinCrc32Pro_checksum_calculate
 * 05may2020,tk MICRO-2364/PR#27563 Added missing SOURCE_BEGIN
 */
/*ce
 * \file
 * \brief ISO CRC32 checksum calculation.
 */
 ...
 */
RTI_PRIVATE const RTI_UINT32 Crc32Lookup[8][256] =
{
  {
...
/*ci
 * \brief Calculate CRC-32 checksum for buf
 *
 * \details
 * Performs a fast CRC-32 with Slicing-by-8 algorithm. The input buffer is
 * aligned to a multiple of 4 bytes.
 *
 * \param[in]   context
                                        ...
                                        RTPS_Checksum_T *checksum)
{
    RTI_UINT32 crc = 0;
    RTI_UINT32 length = 0;
    const RTI_UINT32 *current_word = NULL;
    ...
    /* Previous checksum ^ 0xFFFFFFFF */
    crc = ~checksum->checksum32;

    /* This assumes that the input buffer is aligned to 32 bits, which is
     * the case in Micro.
     */
    OSAPI_Compiler_reinterpret_constcast(current_word,buf[0].pointer,uint32_ptr);
    ...
    while (length >= 8)
    {
        ...
        RTI_UINT32 one = *current_word++ ^ crc;
        RTI_UINT32 two = *current_word++;
...
#else
        RTI_UINT32 one = *current_word++ ^ CRC32_swap(crc);
        RTI_UINT32 two = *current_word++;
     ...
     */
    current_byte = (const RTI_UINT8*)current_word;
    while (length-- > 0)
    {
        crc = (crc >> 8) ^ Crc32Lookup[0][(crc & 0xFF) ^ *current_byte++];
    }
