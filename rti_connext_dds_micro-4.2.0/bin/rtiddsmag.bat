...
for /f tokens^=2-5^ delims^=.-_^" %%j in ('%javaBin% -fullversion 2^>^&1') do set "javaVersion=%%j%%k%%l%%m"
if NOT "%javaVersion:~0,2%" == "17" (
    @echo on
    echo "warning: Micro Application Generator has not been tested with Java %javaVersion%. Unexpected behavior may occur."
    @echo off
)
