...
typedef enum
{
    ...
    /*ci
     * \brief The queue entry corresponding to the SN is being used by
     *        an external user and cannot be evicted regardless of the
     *        cache state.
     */
    DDSHST_WRITEREVENT_KIND_ENTRY_STATE_QUEUED,

    /*ci
     * \brief The queue entry corresponding to the SN is no longer used
     *        by an external user and may can be evicted depending on the
     *        overall cache state.
     */
    ...
    DDSHST_WRITEREVENT_KIND_ENTRY_STATE_SEND_COMPLETED,
     ...
     /*ci
     * \brief Liveliness was asserted by the user.
     */
    DDSHST_WRITEREVENT_KIND_LIVELINESS_ASSERTED

#if DDS_FILTERING_ENABLED
    /*ci
     * \brief Peek in the history cache for the next available sequence number
     *        greater than or equal to the given sequence number.
     */
    ,DDSHST_WRITEREVENT_KIND_PEEK_NEXT_SN
#endif /* DDS_FILTERING_ENABLED */
} DDSHST_WriterEventKind_T;

#if DDS_FILTERING_ENABLED
struct DDSHST_WriterPeekNextEvent
{
    /*ci
     * \brief The starting sequence number to peek from. If an entry is found
     *        then this is updated to the next available sequence number.
     */
    struct REDA_SequenceNumber *sn;

    /*ci
     * \brief Whether to look for only historical samples or not.
     */
...
#if DDS_FILTERING_ENABLED
        struct DDSHST_WriterPeekNextEvent peek_next;
#endif /* DDS_FILTERING_ENABLED */
    } data;
    /*ci
      * \brief Flag to indicate if the
      *        event was ignored.
      */
    RTI_BOOL event_ignored;
};
...
typedef struct DDSHST_WriterSample
{
    ...
    /*ci
     * \brief Index that can be used to index a sample in a writer history.
     * \brief Index that can be used to index a sample in a writer history.
     */
    RTI_UINT32 sample_index;
...
struct DDSHST_WriterProperty
{
    ...
    /*ci
     * \brief type plugin for the writer.
     */
...
/*ci
 * \brief Base-class properties for the DDS writer history cache factory.
 */
struct DDSHST_WriterFactoryProperty
{
     /*ci
...
/*ci
 * \def DDSHST_WriterFactoryProperty_INITIALIZER
 * \brief Constant to initialize \ref DDSHST_WriterFactoryProperty
 */
#define DDSHST_WriterFactoryProperty_INITIALIZER \
{\
...
typedef enum
{
    ...
    /*ci
     * \brief The key was removed due to being deleted
     */
    DDSHST_WRITER_KEY_REMOVED_DELETED
...
{
    /*ci
     * \brief The empty entry, used when an entry is only registered
     */
    DDSHST_WRITER_ENTRY_NONE,
...
/*ci
 * \brief The current state of an instance in the instance cache if it exists
 */
struct DDSHST_InstanceState
{
    /*ci The number of samples owned by the instance
     */
    RTI_INT32 sample_count;

    /*ci The type of the last sample added for the instance
     */
    DDSHST_WriterEntryKind_T last_sample_kind;
};
...
/*ci
 * \brief Possible writer cache errors when an error is returned
 */
typedef enum
{
    /*ci
     * \brief No error, used for initialization
     */
    DDSHST_WRITER_ERROR_NONE,

    /*ci
     * \brief The reason for the error was due to timeout on sample resources
     */
    DDSHST_WRITER_ERROR_TIMEOUT,

    /*ci
     * \brief The reason for the error is running out of instance
     */
    DDSHST_WRITER_ERROR_OUT_OF_INSTANCE_RESOURCES,

    /*ci
     * \brief The reason for the error is running out of sample resources
     */
    DDSHST_WRITER_ERROR_OUT_OF_SAMPLE_RESOURCES,

    /*ci
     * \brief The reason for the error is attempting to write a sample
     *        with an earlier timestamp.
     */
    DDSHST_WRITER_ERROR_SOURCE_TIMESTAMP_ORDER,

    /*ci
     * \brief The reason for the error is attempting to write a sample
     *        with a key that did not already exist.
     */
    DDSHST_WRITER_ERROR_UNKNOWN_KEY,

    /*ci
     * \brief The reason for the error is fialure to block
     */
    DDSHST_WRITER_ERROR_BLOCKING

} DDSHST_WriterErrorKind_T;
 ...
 * \param[in]  source_ts  The source time stamp this instance was registered.
 * \param[out] reason     Additional error code in case NULL is returned.
 *
...
(*DDSHST_Writer_get_entryFunc)(struct DDSHST_Writer *self,
                           ...
                           DDSHST_WriterErrorKind_T *reason)
)
...
/*ci
 * \brief Definition of \ref DDSHST_WriterI get_instance_state method
 *
 * \details
 * Return the state of an instance if it exists
 *
 * \param[in]  self  The history cache
 * \param[in]  key   The key to retrieve state for
 * \param[out] state The current state of an instance, if found
 *
...
(*DDSHST_Writer_get_instance_stateFunc)(struct DDSHST_Writer *self,
                                        const DDS_InstanceHandle_t *const key,
                                        struct DDSHST_InstanceState *state);

/*ci
 * \brief Definition of \ref DDSHST_WriterI unregister_key method
 *
 * \details
 ...
 */
typedef DDSHST_ReturnCode_T
(*DDSHST_Writer_unregister_keyFunc)(struct DDSHST_Writer *self,
                                    const DDS_InstanceHandle_t *const key);
...
struct DDSHST_WriterI
{
    ...
    /*ci
     * \brief Method to retrieve the instance state
     */
    DDSHST_Writer_get_instance_stateFunc get_instance_state;

    /*ci
     * \brief Method to unregister a key
     */
 ...
 *                         Otherwise return NULL if the key does not exist.
 * \param[in]  ts_         The source time stamp for the sample
 * \param[out] reason_     Additional error code in case NULL is returned.
 *
 * \return A reference to new entry if there is space, NULL otherwise
 */
#define DDSHST_Writer_get_entry(self_,key_,kind_,assert_key_,ts_,reason_) \
    ((struct DDSHST_WriterI*)(\
            (self_)->_parent._intf))->get_entry(self_,key_,kind_,\
                                                assert_key_,ts_,reason_)
 ...
 * \paran[in] ts_   The timestamp of the operation
 *
 * \return DDSHST_RETCODE_SUCCESS on success, one of \ref
 *         DDSHST_ReturnCode_T on failure
 */
#define DDSHST_Writer_register_key(self_,key_,ts_) \
    ((struct DDSHST_WriterI*)(\
            (self_)->_parent._intf))->register_key(self_,key_,ts_)
...
/*ci
 * \brief Wrapper to call \ref DDSHST_WriterI  ->get_instance_state method
 *
 * \details
 * Return the state of in instance if it exists
 *
 * \param[in]  self_  The history cache
 * \param[in]  key_   The key to retrieve state for
 * \param[out] state_ The current state of an instance, if found
 *
 * \return DDSHST_RETCODE_NOT_EXISTS if the instance does not exists,
 *         DDSHST_RETCODE_OK if the instance exists
 */
#define DDSHST_Writer_get_instance_state(self_,key_,state_) \
    ((struct DDSHST_WriterI*)(\
            (self_)->_parent._intf))->get_instance_state(self_,key_,state_)

/*ci
 * \brief Wrapper to call \ref DDSHST_WriterI  ->unregister_key method
 *
 * \details
 * Unregister a key from the history. Note that this is not the same as a DDS
 * unregister_instance. Unregistering a key simple deletes it from the
 ...
 *
 * \param[in]  self_  The history cache
 * \param[in]  key_   The key to unregister
 *
 * \return DDSHST_RETCODE_OK on success, DDSHST_RETCODE_NOT_EXISTS if the
 *         instance does not exist.
 */
