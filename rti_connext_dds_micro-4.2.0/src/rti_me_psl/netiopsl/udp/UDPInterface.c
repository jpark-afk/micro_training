 ...
 */
#if defined(RTI_UNIX) || defined(RTI_VXWORKS) || defined(RTI_FREERTOS) || defined(RTI_THREADX)
...
UDP_Interface_create_socket(const struct UDP_Interface *udp_intf)
{
 ...
 */
MUST_CHECK_RETURN RTI_PRIVATE RTI_BOOL
UDP_Interface_set_multicast_loopback_option(RTI_SOCKET sock,RTI_UINT8 option)
{
    if (netiosock_setsockopt(sock,IPPROTO_IP,
 ...
 */
MUST_CHECK_RETURN RTI_PRIVATE RTI_BOOL
UDP_Interface_enable_multicast_loopback(RTI_SOCKET sock)
{
 ...
 */
MUST_CHECK_RETURN RTI_PRIVATE RTI_BOOL
UDP_Interface_disable_multicast_loopback(RTI_SOCKET sock)
{
                                      ...
                                      const struct UDP_NetworkIfInfo *if_data)
{
    ...
    s = UDP_Interface_create_socket(src_intf);
    if (s != -1)
    {
        /* The locator kind is not of interest in this context and it is
         * used to save the socket bound to the outgoing interface.
         */
#ifdef RTI_WIN32
        an_address->kind = (RTI_INT32)s;
#else
        an_address->kind = s;
#endif
        if (netiosock_setsockopt(s, IPPROTO_IP,
...
UDP_Interface_multicast_interface_finalize(struct UDP_Interface *src_intf)
{
    ...
    for (index = 0; index < length; index++)
    {
        ...
        netiosock_close(an_address->kind);
    }
...
UDP_Interface_set_multicast_wakeup(struct UDP_Interface *self,RTI_UINT8 enable)
{
    ...
    {
        return UDP_Interface_set_multicast_loopback_option(self->send_socket,enable);
    }
    ...
    for (index = 0; index < length; index++)
    {
        ...
        if (!UDP_Interface_set_multicast_loopback_option(an_address->kind,enable))
        {
            return RTI_FALSE;
        }
    }
                                  ...
                                  struct NETIOBindProperty *property)
{
        ...
        && !src_intf->factory->property->disable_multicast_interface_select)
    {
         ...
         */
        if (netiosock_setsockopt(src_intf->send_socket, IPPROTO_IP,
             IP_MULTICAST_IF,(char *)&src_intf->mc_bind_address.value.ipv4.address,
                      ...
                      IOVEC_MSG *message)
{
    ...
    {
        bytes_sent = (RTI_SIZE_T)netiosock_sendto(sock,
    ...
    else
    {
        ...
        /* coverity[cert_pos39_c_violation : FALSE] */
        bytes_sent = netiosock_sendmsg(sock, message, 0);
#endif
    }
