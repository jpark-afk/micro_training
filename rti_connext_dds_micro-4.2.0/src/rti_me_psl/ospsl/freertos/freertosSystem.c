...
OSAPI_SystemFreeRTOS_generate_uuid(struct OSAPI_SystemUUID *uuid_out)
{
    ...
    RTI_UINT32 self_uint = OSAPI_System_fv_UuidCounter++;
    uuid_out->value[0] = ((self_uint>>24)&0x000000ffU) |
                         ((self_uint>>8)&0x0000ff00U)  |
                         ((self_uint<<24)&0xff000000U) |
                         ((self_uint<<8)&0x00ff0000U);
#else
    uuid_out->value[0] = OSAPI_System_fv_UuidCounter++;
#endif
    uuid_out->value[1] = self_uint;
