...
struct HelloWorldUdpTransform
{
    ...
    NETIO_Packet_T send_packet;

    NETIO_Packet_T recv_packet;
    ...
    unsigned char *serialized_gather_buffer;

    unsigned int serialized_gather_buffer_length;
...
/* Maximum length of a serialized sample + RTPS overhead. Micro doesn't
 * aggregate samples so at most one DATA sample + protocol messages.
 */
#define MAX_SERIALIZED_OVERHEAD (256)
                              ...
                              const struct UDP_TransformProperty *const property)
{
    ...
    if (HelloWorldTypeSupport_serialize_data_to_cdr_buffer(NULL,&t->serialized_gather_buffer_length,NULL) != DDS_RETCODE_OK)
    {
        printf("Failed to get max serialized size for HelloWorld type\n");
        (void)HelloWorldEncryption_finalize(t->cypher_context);
    ...
    t->serialized_gather_buffer_length += MAX_SERIALIZED_OVERHEAD;

    OSAPI_Heap_allocate_buffer((char**)&t->serialized_gather_buffer,
                               t->serialized_gather_buffer_length,
                               OSAPI_ALIGNMENT_DEFAULT);

    if (t->serialized_gather_buffer == NULL)
    {
        (void)HelloWorldEncryption_finalize(t->cypher_context);
        OSAPI_Heap_free_buffer((char**)t->buffer);
        OSAPI_Heap_free_struct(t);
...
HelloWorldUdpTransform_delete(struct HelloWorldUdpTransform *t)
{
    if (t != NULL)
    {
        ...
        if (t->serialized_gather_buffer != NULL)
        {
            OSAPI_Heap_free_buffer((char**)t->serialized_gather_buffer);
        }
                                ...
                                RTI_INT32 *ec)
{
    ...
    *ec = 0;
    ...
    if (!NETIO_Packet_initialize_from(&self->send_packet,
                                      in_packet,
                                      self->buffer,
                                      (RTI_SIZE_T)self->buffer_length,
                                      0,
                                      (RTI_SIZE_T)self->buffer_length))
    {
        return RTI_FALSE;
    ...
    *out_packet = &self->send_packet;

    buf_ptr = NETIO_Packet_get_head(&self->send_packet);
    from_buf_ptr = NETIO_Packet_get_head(in_packet);

    input_length = (int)NETIO_Packet_get_payload_length(in_packet);
    ...
    if (!NETIO_Packet_set_tail(&self->send_packet, output_length - self->buffer_length))
    {
        return RTI_FALSE;
    }
                                ...
                                RTI_INT32 *ec)
{
    ...
    unsigned char *from_buf_ptr;
    int input_length, output_length;
    unsigned char *tmp_buffer_ptr = NULL;
    ...
    *ec = 0;
    ...
    if (!NETIO_Packet_initialize_from(&self->recv_packet,
                                      in_packet,
                                      self->buffer,
                                      (RTI_SIZE_T)self->buffer_length,
                                      0,
    ...
    *out_packet = &self->recv_packet;


    pktbuf = in_packet->head_pbuf;
    tmp_buffer_ptr = self->serialized_gather_buffer;

    while (pktbuf != NULL)
    {
        if (((tmp_buffer_ptr - self->serialized_gather_buffer) +
                NETIO_PacketBuffer_get_length(pktbuf)) > self->serialized_gather_buffer_length)
        {
            printf("Out of space when copying into gather buffer\n");
            return RTI_FALSE;
        }

        OSAPI_Memory_copy(tmp_buffer_ptr,
                          NETIO_PacketBuffer_get_head(pktbuf),
                          NETIO_PacketBuffer_get_length(pktbuf));

        tmp_buffer_ptr += NETIO_PacketBuffer_get_length(pktbuf);

        pktbuf = pktbuf->_next;
    }

    buf_ptr = NETIO_Packet_get_head(&self->recv_packet);
    output_length = self->buffer_length;

    from_buf_ptr = self->serialized_gather_buffer;
    input_length = (int)(tmp_buffer_ptr - self->serialized_gather_buffer);
    ...
    if (!NETIO_Packet_set_tail(&self->recv_packet, output_length - self->buffer_length))
    {
        return RTI_FALSE;
    }
