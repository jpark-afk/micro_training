...
struct CDR_Stream
{
    ...
    /*ci \brief Where current stream started */
    char *start_ptr;
...
struct CDR_Stream_t
{
    /* Empty definition so that the type is opaque in Doxygen */
};
#else
#define CDR_Stream_t CDR_Stream
#endif
 ...
 */
CDRDllExport void
CDR_Stream_align(struct CDR_Stream_t *stream,RTI_UINT8 alignment);
...
/*e \dref_CDR_Stream_t_check_size
 */
MUST_CHECK_RETURN CDRDllExport RTI_BOOL
CDR_Stream_check_size(struct CDR_Stream_t *me, RTI_UINT32 size); 

/*e \dref_CDR_Stream_t_get_current_position_ptr
 */
MUST_CHECK_RETURN CDRDllExport char*
...
/*e \dref_CDR_Stream_t_is_byte_swapped
 */
MUST_CHECK_RETURN CDRDllExport RTI_BOOL
