...
/*e \dref_OSAPIHeapGroupDocs
 */
 
/*i \defgroup OSAPI_HeapClass OSAPI Heap
 *  \ingroup OSAPIModule
 */
 ...
 * 
 * This operation is implemented as a macro in order to support the
 * specification of a "type" argument (converted to a size value using the
 * sizeof operator).
 * 
 * \param[out] pointer  Pointer to the allocated structure.
 * \param[in]  type     Type of the structure to allocate.
 *
 * \sa \ref OSAPI_Heap_free_struct
 * 
*/
OSAPIDllExport void
OSAPI_Heap_allocate_struct(MACRO_TYPE **pointer, MACRO_TYPE type);
 ...
 * 
 * \details
 * space (that was previously allocated with \ref OSAPI_Heap_allocate_struct)
 * to the heap.
 * 
 * This operation is implemented as a macro in order to support the
 * specification of a "type" argument. 
 * 
 * \param[in] pointer Pointer to region previous allocated
 * 
 * \sa \ref OSAPI_Heap_allocate_struct
 */
OSAPIDllExport void
