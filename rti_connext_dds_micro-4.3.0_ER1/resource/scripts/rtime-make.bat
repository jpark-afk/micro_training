...
set "RESOURCE_ARCH_DIR=%RESOURCE_DIR%\architectures"
set "MODULE_ARCH_PATH=%MODULE_PATH%\\architectures"
...
if "%DO_HELP%" == "1" (
    if "%ARCH%" == "" (
        ...
        "%CMAKE_BIN%" -DTARGET_HELP:BOOLEAN=1 -DCMAKE_MODULE_PATH="%MODULE_ARCH_PATH%" -DRTIME_TARGET_NAME=%NAME% -P "%RESOURCE_ARCH_DIR%\%ARCH%.tc"
        goto :eof
    )
    exit /b
)
...
if "x%ARCH%" == "x" (
    ...
    if NOT "%ARCH%" == "self" (
        if not exist "%RESOURCE_ARCH_DIR%\%ARCH%.tc" (
            call :print_error "Unknown target %ARCH% (%RESOURCE_ARCH_DIR%\%ARCH%.tc) listed. use --list for a list of available targets."
            exit /b
        ) else (
            set TOOLCHAIN_FILE=-DCMAKE_TOOLCHAIN_FILE="%RESOURCE_ARCH_DIR%\%ARCH%.tc"
        )
    )
...
if "%CMAKE_G%" == "" (
    %CMD_ECHO% "%CMAKE_BIN%" -DCMAKE_MODULE_PATH="%MODULE_ARCH_PATH%" %TOOLCHAIN_FILE% %SYSTEM_FILE% %CMAKE_OPTIONS% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% %GIT_BRANCH% -DRTIME_TARGET_NAME=%NAME% -B%CMAKE_ROOT% %ENABLE_QUACK% -H%SOURCE_DIR%
) else (
    %CMD_ECHO% "%CMAKE_BIN%" -DCMAKE_MODULE_PATH="%MODULE_ARCH_PATH%" %TOOLCHAIN_FILE%  %SYSTEM_FILE% -G "%CMAKE_G%" %CMAKE_OPTIONS% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% %GIT_BRANCH% %ENABLE_QUACK% -DRTIME_TARGET_NAME=%NAME% -B%CMAKE_ROOT% -H%SOURCE_DIR%
)
...
:listarch
    ...
    echo.
    for %%f in ("%RESOURCE_ARCH_DIR%\*.tc") do echo %%~nf
exit /b
