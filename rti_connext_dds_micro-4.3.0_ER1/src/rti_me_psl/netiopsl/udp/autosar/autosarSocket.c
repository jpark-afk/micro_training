...
/*i
 * \brief External function provided by the application to signal that a new
 * UDP packet has been received and is ready for processing.
 */
extern FUNC(void, SOAD_CODE) DdsCddRxIndication(void);
    ...
    socklen_t addr_len)
{
    ...
    if (OSAPI_System_gv_PortProperty->use_socket_owner)
    {
        static boolean socket_bind = RTI_FALSE;
        /* bind socket to a port */
        if (socket_bind == RTI_FALSE)
        {
            uint16 PortPtr_a = TCPIP_PORT_ANY;
            Std_ReturnType ret = TcpIp_Bind((TcpIp_SocketIdType)socket,
                    OSAPI_System_gv_PortProperty->send_local_addr_id,
                    &PortPtr_a);
            if (ret == E_OK)
            {
                socket_bind = RTI_TRUE;
            }
        }
        if (socket_bind == RTI_TRUE)
        {
