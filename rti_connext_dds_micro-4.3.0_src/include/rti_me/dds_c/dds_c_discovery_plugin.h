...
/*ci
 *  brief Definition of the \ref NDDS_DiscoveryI write_announcement method
 *
 * \param[in] discovery_plugin  The discovery plugin
 * \param[in] participant       Not used
 * \param[in] participant_data  The participant announcement data to write
 *
 * \return DDS_RETCODE_OK on success, DDS_RETCODE_ERROR on failure
 */
FUNCTION_MUST_TYPEDEF(
DDS_ReturnCode_t 
(*NDDS_Discovery_Plugin_WriteAnnouncement)(
        struct NDDS_Discovery_Plugin *const discovery_plugin,
        DDS_DomainParticipant *const participant,
...
struct NDDS_DiscoveryI
{
    ...
    /*ci
     * \brief Method called to write a participant announcement
     */
    NDDS_Discovery_Plugin_WriteAnnouncement
    write_announcement;
};
...
#define NDDS_Discovery_Plugin_on_write_announcement(self_,p_,data_) \
((struct NDDS_DiscoveryI*)((self_)->_parent._intf))->write_announcement(self_,p_,data_)

#define NDDS_Discovery_Plugin_has_on_write_announcement(self_) \
(((struct NDDS_DiscoveryI*)(self_)->_parent._intf)->write_announcement != NULL)
