 ...
 *        has already been registered or because the provided interface did not
 *        contain the required function pointers.
 *
 * \ingroup DDSFilterLogCodesClass
 */
 ...
 *
 * \details
 * For strings that are configured by the user in their QoS, this is an error
 * that indicates that the resource limits configured were not sufficient to
 * hold the string. For content filters received through discovery, this is
...
/*e
 * \brief Content filtering is not supported on this data type because its
 ...
 * with \idref_SqlFilterProperty_predicate_max_count_per_expression in
 * \idref_SqlFilterProperty when registering the SQL content filter class
 * using \idref_DomainParticipant_register_contentfilter.
 *
 * \ingroup DDSFilterLogCodesClass
 */
...
/*e
 * \brief The builtin DDS SQL filter was registered with an invalid property
 *
 * \ingroup DDSFilterLogCodesClass
 */
#define DDS_FILTER_LOG_SQL_INVALID_PROPERTY_EC       (DDS_FILTER_LOG_BASE + 110)
#define DDS_FILTER_LOG_SQL_INVALID_PROPERTY(level_,value_) \
OSAPI_LOG_ENTRY_ADD_1INT((level_),DDS_FILTER_LOG_SQL_INVALID_PROPERTY_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM,"value",(value_))
