 ...
 *
 * Copyright (c) 2012-2026 Real-Time Innovations, Inc.
 *
 * All rights reserved.
 *
...
/*e
 * \brief Packet payload length overflow detected
 *
 * \details NETIO_Packet_get_payload_length() detected an integer overflow
 * while accumulating lengths from packet buffer linked list.
 *
 * \ingroup NETIOLogCodesClass
 */
#define NETIO_LOG_PACKET_OVERFLOW_EC                         (UDP_LOG_BASE + 30)
#define NETIO_LOG_PACKET_OVERFLOW(level_) \
OSAPI_LOG_ENTRY_ADD((level_),NETIO_LOG_PACKET_OVERFLOW_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM)
...
/*e \brief Failed to set socket priority
 * \details A socket priority could not be set due to system error
 * \ingroup NETIOLogCodesClass
 */
#define UDP_LOG_SOCKET_PRIORITY_EC                        (UDP_LOG_BASE + 251)
#define UDP_LOG_SOCKET_PRIORITY(level_,priority_,sysrc_) \
OSAPI_LOG_ENTRY_ADD_2INT((level_),UDP_LOG_SOCKET_PRIORITY_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM ,\
        "priority",(priority_),"sysrc",(sysrc_))

/*e \brief  IP_TOS is not supported
 *
 * \details
 * The system does not support setting the IP_TOS
 * (Type of Service) field for UDP sockets.
 *
 * \ingroup NETIOLogCodesClass
 */
#define UDP_LOG_TOS_NOT_SUPPORTED_EC                        (UDP_LOG_BASE + 252)
#define UDP_LOG_TOS_NOT_SUPPORTED(level_) \
OSAPI_LOG_ENTRY_ADD((level_),UDP_LOG_TOS_NOT_SUPPORTED_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM)
...
/*e
 * \brief Failed to allocate memory for priority map
 *
 * \ingroup NETIOLogCodesClass
 */

#define UDP_PRIORITY_MAP_ALLOC_EC                        (UDP_LOG_BASE + 310)
#define UDP_PRIORITY_MAP_ALLOC(level_) \
OSAPI_LOG_ENTRY_ADD((level_),UDP_PRIORITY_MAP_ALLOC_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM)


/*e
 * \brief Invalid priority mapping specified
 * \details
 * The specified priority mapping is invalid, the low priority mapping must be
 * less than or equal to the high priority mapping.
 * \ingroup NETIOLogCodesClass
 */
#define UDP_PRIORITY_MAP_INVALID_MAPPING_EC                        (UDP_LOG_BASE + 311)
#define UDP_PRIORITY_MAP_INVALID_MAPPING(level_,priority_l, priority_h) \
OSAPI_LOG_ENTRY_ADD_2INT((level_),UDP_PRIORITY_MAP_INVALID_MAPPING_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM ,\
        "priority_mapping_low",(priority_l),"priority_mapping_high",(priority_h))


/*e
 * \brief Negative priority mapping values specified
 * \details
 * The specified priority mapping values are invalid, both low and high priority 
 * mapping values must be non-negative (0-255).
 * \ingroup NETIOLogCodesClass
 */
#define UDP_PRIORITY_MAP_NEGATIVE_MAPPING_EC                        (UDP_LOG_BASE + 312)
#define UDP_PRIORITY_MAP_NEGATIVE_MAPPING(level_,priority_l, priority_h) \
OSAPI_LOG_ENTRY_ADD_2INT((level_),UDP_PRIORITY_MAP_NEGATIVE_MAPPING_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM ,\
        "priority_mapping_low",(priority_l),"priority_mapping_high",(priority_h))


/*e
 * \brief Trasnport priority ignored
 * \details
 * The DataReader or DataWriter trasnport priority is ignored by the UDP transport because the priority mask is not configured.
 */
#define UDP_PRIORITY_MAP_PRIORITY_IGNORED_EC                        (UDP_LOG_BASE + 313)
#define UDP_PRIORITY_MAP_PRIORITY_IGNORED(level_) \
OSAPI_LOG_ENTRY_ADD((level_),UDP_PRIORITY_MAP_PRIORITY_IGNORED_EC,\
        OSAPI_LOG_MSG_PN_X2_STD_PARAM)
#endif /* netio_log_h */
