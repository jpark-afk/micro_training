...
/*ci
 * \brief Trigger socket recovery after FullCom / IP-address re-assignment.
 *
 * Acquires fresh socket IDs from TcpIp, re-binds them to the same local
 * ports active at degradation time, and re-registers recvnotify callbacks
 * on all registered UDP interfaces.
 * Idempotent: returns E_OK immediately if already healthy or if a
...
/*ci
 * \brief MICRO-12928: Handle a TcpIp socket event (e.g. TCPIP_UDP_CLOSED).
 *
 * When the event is TCPIP_UDP_CLOSED the corresponding receive port entry
 * is marked closed so that the next \ref NETIO_Autosar_on_ip_assigned call
 * will re-open and re-bind it.
 *
 * \param[in] socket_id    The TcpIp socket whose event is being reported.
 * \param[in] socket_event The event type (e.g. TCPIP_UDP_CLOSED).
 *
 * \return E_OK on success, E_NOT_OK on error.
 */
FUNC(Std_ReturnType, SOAD_CODE)
...
/*ci
 * \brief Update the internal IP-assignment bitmap.
 *
 * Must be called from the \c LocalIpAddrAssignmentChg socket-owner callback
 * for every IP address state change so that the PSL can correctly gate
 * socket operations on address availability.
 *
 * \param[in] local_addr_id The local address ID whose state changed.
 * \param[in] state         The new IP address state (e.g. TCPIP_IPADDR_STATE_ASSIGNED).
 */
FUNC(void, SOAD_CODE)
