...
RTPS_Guid_equals(
        const struct RTPS_Guid* const a,
...
#define RTPS_VENDOR_ID_INITIALIZER {0,0}
...
/*ci \brief The maximum number of encapsulations that can be included in an
 *          extended locator.
 */
...
/*ci
 * \brief
 * Serialize two octets to stream buffer
 *
 * \param[inout]    Stream Serialization stream
 * \param[in] in    Two Octets to serialize
 * \param[in] param Unused
 *
 * \return RTI_TRUE on success, RTI_FALSE on failure
 */
...
/*ci
 * \brief
 * Deserialize two octets from stream buffer
 *
 * \param[in] stream Deserialization stream
 * \param[inout] out Two deserialized octets
 * \param[in] param  Unused
 *
 * \return RTI_TRUE on success, RTI_FALSE on failure
 */
...
/*ci
 * \brief
 * Length in bytes of serialized octet pair
 *
 * \param[in] size Current length of serialized buffer
 *
 * \return Number of bytes of serialized octet pair
 */
MUST_CHECK_RETURN RTPSDllExport RTI_UINT32
...
/*ci \brief Data K-flag for DATA_FRAG submessage */
#define RTPS_DATAFRAG_FLAGS_K 0x4
...
struct RTPS_InterfaceProperty
{
    ...
    /*ci \brief The flow-controller to use (may be NULL)
     */
    NETIO_FlowController *netio_fc;

    RTPS_InterfaceTrustProperty trust_property;

    /*ci \brief Downstream transport priority
     */
    RTI_INT32 transport_priority;

    /*ci \brief The external RTPS interface for the Participant
     */
