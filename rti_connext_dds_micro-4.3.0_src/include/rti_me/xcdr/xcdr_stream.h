...
/*
 * The maximum size of a CDR stream that is sufficiently aligned
 */
#define RTI_XCDR_MAX_ALIGNED_SERIALIZED_SIZE \
    RTIOsapiAlignment_alignUInt64Up( \
            RTI_XCDR_MAX_SERIALIZED_SIZE - MIG_RTPS_SUBMESSAGE_ALIGNMENT, \
...
/* The current number of CDR types, CDR1 and CDR2 */
#define RTI_XCDR_TK_NUM_CDR_TYPES 2

/*
 * The number of entries in the RTIXCdr_TCKind_g_primitiveSizes array so that
 * we can index into the array to get a primitive type's size without calling a
 * function and having to do a switch
 */
#define RTI_XCDR_TK_NUM_PRIMITIVE_SIZES 25
 ...
 */
extern RTIXCdrDllVariable const RTIXCdrUnsignedLong
        RTIXCdr_TCKind_g_primitiveCdrSizes[RTI_XCDR_TK_NUM_CDR_TYPES]
                                          [RTI_XCDR_TK_NUM_PRIMITIVE_SIZES];
...
extern RTIXCdrDllVariable const RTIXCdrAlignment
        RTIXCdr_TCKind_g_primitiveCdrAlignments
