###############################################################################
# (c) Copyright, Real-Time Innovations 2025-2026
#
# All rights reserved.
...
IF(NOT FORGE_EXECUTABLE)
    IF(RTIME_DEVTREE)
        MESSAGE(FATAL_ERROR
            ...
            "tools were not found. Install forge, or configure with "
            "-DRTI_BUILD_FORGE_UNITTESTS=OFF to skip it.")
    ELSE()
...
#
# This function:
# 1. Generates a Forge profile using 'forge profile create'
...
#   SUBSTRATE_VERSION: (Optional) Substrate version string. Accepts a pinned release
#                      (e.g. '0.1.1'), 'latest', or a git branch using the 'branch:'
#                      prefix (e.g. 'branch:feature/FOUNDRY-151'). Passed to
#                      'forge run' via --substrate-version.
...
FUNCTION(ADD_FORGE target)
    SET(options DEBUG)
    SET(oneValueArgs PROGRAM ARCH SUBSTRATE OUTPUT_DIR PROFILE_OUTPUT EXTENDS SUBSTRATE_VERSION)
    ...
    IF(NOT FORGE_EXECUTABLE)
        MESSAGE(FATAL_ERROR "Forge not found, cannot add forge target ${target}")
    ENDIF()
    ...
    # but good to be explicit if needed, though COMMAND usually handles it or we rely on forge)
    # Actually, 'forge' might expect the dir to exist or create it. 'forge run' creates output dir.
    # We'll let the tools handle it or create it via command.
    FILE(MAKE_DIRECTORY ${FORGE_OUTPUT_DIR})

    # 1. Generate Profile
    SET(FORGE_PROFILE_CMD ${FORGE_EXECUTABLE} profile create
        --name ${target}
        --output ${FORGE_PROFILE_OUTPUT}
    )

    IF(FORGE_EXTENDS)
        LIST(APPEND FORGE_PROFILE_CMD --extends ${FORGE_EXTENDS})
    ENDIF()

    IF(FORGE_LIBS)
        LIST(APPEND FORGE_PROFILE_CMD --libs ${FORGE_LIBS})
    ENDIF()

    IF(FORGE_LIB_DIRS)
        LIST(APPEND FORGE_PROFILE_CMD --lib-dirs ${FORGE_LIB_DIRS})
    ENDIF()

    ADD_CUSTOM_COMMAND(OUTPUT ${FORGE_PROFILE_OUTPUT}
        COMMAND ${FORGE_PROFILE_CMD}
    ...
    # Add substrate version if SUBSTRATE_VERSION is defined
    IF(FORGE_SUBSTRATE_VERSION)
        LIST(APPEND FORGE_CMD --substrate-version ${FORGE_SUBSTRATE_VERSION})
    ENDIF()
