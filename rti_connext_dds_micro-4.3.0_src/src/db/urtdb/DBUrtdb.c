 ...
 */
#define URTDB_RECORDER_HEADER_SIZE (sizeof(URTDB_RecordHeader))
                   ...
                   void *arg0,void *arg1)
{
    ...
    *eh = cursor;
    ...
    else if (opcode == DB_SELECTOPCODE_FROM)
    {
                   ...
                   struct OSAPI_Mutex *const shared_lock)
{
    ...
    OSAPI_PRECONDITION((db == NULL) || (*db != NULL) || (db_property == NULL),
    ...
    *db = NULL;
    ...
    REDA_CircularList_init(&urtdb->tables);
    urtdb->lock = NULL;
    urtdb->lock_mode = db_property->lock_mode;

    if (urtdb->lock_mode > DB_LOCK_LEVEL_SHARED)
    {
    ...
    else if (urtdb->lock_mode == DB_LOCK_LEVEL_SHARED)
    {
...
DB_Database_delete(DB_Database_T db)
{
    ...
    table = (struct DB_Table*)REDA_CircularList_get_first(&db->tables);

    while (!REDA_CircularList_node_at_head(&db->tables,&table->node))
    {
    ...
    if (!REDA_CircularList_is_empty(&db->tables))
    {
        DB_LOG_TABLES_INUSE(OSAPI_LOGKIND_ERROR)
        return DB_RETCODE_EXISTS;
    }

    if ((db->lock_mode != DB_LOCK_LEVEL_SHARED) && (db->lock != NULL))
    {
...
OSAPI_Mutex_T*
DB_Database_get_lock(DB_Database_T db)
{
    return db->lock;
}
                                            ...
                                            void* dtor_param)
{
    ...
    OSAPI_Heap_allocate_struct(&out_table,struct DB_Table);
    if (out_table == NULL)
    {
    ...
    OSAPI_Memory_zero(out_table,sizeof(struct DB_Table));
...
#if OSAPI_ENABLE_PRECONDITION
    out_table->name = REDA_String_dup(name);
#endif

    if ((ctor != NULL) || (dtor != NULL))
    {
        OSAPI_Heap_allocate_struct(&out_table->ctordtor,struct DB_TableRowCtorDtor);
        if (out_table->ctordtor == NULL)
        {
            DB_LOG_OUT_OF_CTORDTOR(OSAPI_LOGKIND_ERROR)
            retcode = DB_RETCODE_OUT_OF_RESOURCES;
            goto done;
        }
        out_table->ctordtor->user_ctor.ctor = ctor;
        out_table->ctordtor->user_ctor.user_param = ctor_param;
        out_table->ctordtor->user_dtor.dtor = dtor;
        out_table->ctordtor->user_dtor.user_param = dtor_param;
    }
    ...
    if (out_table->ctordtor != NULL)
    {
        out_table->records = REDA_BufferPool_new(
                    name,&pool_property,
                    DDS_DataBase_forward_ctor,
                    (void*)&out_table->ctordtor->user_ctor,
                    DDS_Database_forward_dtor,
                    (void*)&out_table->ctordtor->user_dtor);
    }
    else
    {
        out_table->records = REDA_BufferPool_new(
    ...
    if (out_table->records == NULL)
    {
        if (out_table->ctordtor != NULL)
        {
            OSAPI_Heap_free_struct(out_table->ctordtor);
            OSAPI_Heap_free_struct(out_table);
        }
    ...
    *table = (DB_Table_T) out_table;
    ...
    REDA_CircularList_link_node_after(&db->tables,&out_table->node);
...
DB_Database_delete_table(DB_Database_T db,DB_Table_T table)
{
    ...
    if ((table->records != NULL) && !REDA_BufferPool_delete(table->records))
    {
        DB_LOG_RECORDS_INUSE(OSAPI_LOGKIND_ERROR,DB_Table_get_name(table))
        return DB_RETCODE_EXISTS;
    }
    ...
    if (table->cursors != NULL)
    {
        OSAPI_Heap_free_array(table->cursors);
    }

    if (table->ctordtor != NULL)
    {
        OSAPI_Heap_free_struct(table->ctordtor);
    }

#if OSAPI_ENABLE_PRECONDITION
    if (table->name != NULL)
    {
        REDA_String_free(table->name);
    }
#endif
...
DB_Table_create_record(DB_Table_T table,DB_Record_T *record)
{
    ...
    URTDB_RecordHeader *internal_record;
    ...
    internal_record = (URTDB_RecordHeader*)REDA_BufferPool_get_buffer(
                                                            table->records);
    ...
    URTDB_Record_set_allocated(internal_record);
    *record = (DB_Record_T)URTDB_RECORD_TO_USER_RECORD(internal_record);
    table->records_in_use++;
...
{
    URTDB_RecordHeader *urdb_record = (URTDB_RecordHeader *)record;
    DB_ReturnCode_T retcode = DB_RETCODE_ERROR;
    RTI_INT32 v_index;
    ...
    /* If a record is already inserted, do not re-insert it. This could happen
     * if a record is inserted, the key is changed and then inserted again.
     */
    if (URTDB_Record_is_inserted(urdb_record))
    {
        return DB_RETCODE_RECORD_INSERTED;
    }

    if (!URTDB_Record_is_allocated(urdb_record) &&
        !URTDB_Record_is_removed(urdb_record))
    {
        return DB_RETCODE_INVALID_RECORD;
    }
    ...
    URTDB_Record_set_inserted(urdb_record);
                              ...
                              DB_Table_RecordDtor_T dtor)
{
    ...
    if (URTDB_Record_is_inserted(urdb_record))
    {
        ...
        URTDB_Record_set_removed(urdb_record);
    }

    if (!URTDB_Record_is_inserted(urdb_record) &&
        !URTDB_Record_is_removed(urdb_record) &&
        !URTDB_Record_is_allocated(urdb_record))
    {
        return DB_RETCODE_ERROR;
    }
...
{
    DB_ReturnCode_T retcode = DB_RETCODE_OK;
    URTDB_RecordHeader *existing_record;
    RTI_INT32 v_index;
    ...
    URTDB_Table_update_cursors(table,RTI_TRUE);
    URTDB_Record_set_removed(existing_record);
                      ...
                      const struct DB_IndexProperty *const property)
{
    ...
    {
        table->resource_state |= (DB_INDEX_STATE << j);
        return DB_RETCODE_OUT_OF_RESOURCES;
    }
    ...
    table->resource_state |= (DB_INDEX_INUSE << j);
    *index = a_index;
...
DB_ReturnCode_T
DB_Table_select_from(DB_Table_T table,
                     DB_Index_T index,
                     DB_Cursor_T *eh,
                     DB_Key_T from_key)
{
    OSAPI_PRECONDITION((table==NULL) || (eh==NULL) ||
                       (from_key==NULL),
                        ...
                        OSAPI_Log_entry_add_pointer("table",table,RTI_FALSE);
                        OSAPI_Log_entry_add_pointer("eh",eh,RTI_FALSE);
                        OSAPI_Log_entry_add_pointer("from_key",from_key,RTI_TRUE);)
    ...
    return URTDB_Table_select(table,index,eh,
                              DB_SELECTOPCODE_FROM,from_key,NULL);
}
...
/*
 * This function returns a cursors previously returned by one of the
