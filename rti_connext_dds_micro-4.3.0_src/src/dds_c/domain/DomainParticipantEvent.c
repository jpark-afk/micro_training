...
{
    struct NETIO_Address dw_address = NETIO_Address_INITIALIZER;
...
#if DDS_LIVELINESS_CHANNEL_ENABLED
    if (DDS_LivelinessQosPolicy_is_ipc_liveliness_required(&dr_qos->liveliness))
    {
        if (!DDS_IpcLiveliness_assert(participant))
        {
 ...
 * \param[in] already_enabled If the datareader has already been enabled
 *                            and is instead being updated with a new QoS
 */
RTI_BOOL
...
DomainParticipantEvent_on_before_datareader_deleted(DDS_DataReader *const reader)
{
    ...
    do
    {
        ...
        if (dbrc == DB_RETCODE_OK)
        {
            ...
            DDS_DataWriter_unmatch_local_reader(local_writer,&dr_key,reader);

            DDS_DataReader_unmatch_local_writer(reader,&dw_key,local_writer);
        }
    } while (dbrc == DB_RETCODE_OK);
    ...
    drio = reader->dr_intf;
    if ((drio != NULL) && !DDS_DataReader_is_anonymous(reader))
    {
        dw_cursor = NULL;
        ...
        do
        {
            ...
            if (dbrc == DB_RETCODE_OK)
            {
                DDS_BuiltinTopicKey_from_guid(&dw_key,
                        (DDS_InstanceHandle_t*)&drio_be->key.peer);
    ...
    if (participant->disc_plugin)
    {
        bretval = NDDS_Discovery_Plugin_on_after_local_datareader_deleted(
                participant->disc_plugin,participant,reader);
#if OSAPI_ENABLE_LOG
        if (!bretval)
        {
...
#if DDS_LIVELINESS_CHANNEL_ENABLED
    if (DDS_LivelinessQosPolicy_is_ipc_liveliness_required(&dw_qos->liveliness))
    {
        if (!DDS_IpcLiveliness_assert(participant))
        {
            return DDS_RETCODE_ERROR;
                                    ...
                                    const struct DDS_DataWriterQos *const qos)
{
    ...
    if (participant->disc_plugin)
    {
        if (!NDDS_Discovery_Plugin_on_after_local_datawriter_enabled(
                            participant->disc_plugin,participant,writer,NULL))
        {
    ...
    if (DDS_DomainParticipant_is_ipc_liveliness_enabled(participant) &&
        DDS_IpcLiveliness_update_datawriter_liveliness_timeout(
                            participant->ipc_liveliness) != DDS_RETCODE_OK)
    {
        goto done;
    }
...
DomainParticipantEvent_on_before_datawriter_deleted(DDS_DataWriter *const writer)
{
    ...
    do
    {
        ...
        if (dbrc == DB_RETCODE_OK)
        {
            ...
            DDS_DataReader_unmatch_local_writer(local_reader,&dw_key,writer);

            DDS_DataWriter_unmatch_local_reader(writer,&dr_key,local_reader);
        }
    } while (dbrc == DB_RETCODE_OK);
    DB_Cursor_finish(participant->local_reader_table,dr_cursor);
    ...
    dwio = writer->dw_intf;
    if ((dwio != NULL) && !DDS_DataWriter_is_anonymous(writer))
    {
        cursor = NULL;
    ...
    if (participant->disc_plugin)
    {
        bretval = NDDS_Discovery_Plugin_on_after_local_datawriter_deleted(
                participant->disc_plugin,participant,writer);
#if OSAPI_ENABLE_LOG
        if (!bretval)
        {
 ...
 */
void
DomainParticipantEvent_on_after_datawriter_deleted(
        DDS_DataWriter *const writer)
{
#if DDS_LIVELINESS_CHANNEL_ENABLED
    ...
    /* In case the ipc_liveliness channel has already been deleted */
    if (DDS_DomainParticipant_is_ipc_liveliness_enabled(participant))
    {
        DDS_IpcLiveliness_on_after_datawriter_deleted(participant->ipc_liveliness,writer);
    }
