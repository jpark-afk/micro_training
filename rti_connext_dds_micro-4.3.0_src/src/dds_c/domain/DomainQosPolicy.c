                            ...
                            const struct DDS_DiscoveryQosPolicy* from)
{
    ...
    self->enable_participant_discovery_by_name = from->enable_participant_discovery_by_name;
    self->enable_endpoint_discovery_queue = from->enable_endpoint_discovery_queue;
    self->metatraffic_transport_priority = from->metatraffic_transport_priority;
                                ...
                                const struct DDS_DiscoveryQosPolicy* from)
{
    ...
    if (self->enable_participant_discovery_by_name !=
        from->enable_participant_discovery_by_name)
    {
        return DDS_BOOLEAN_FALSE;
    }

    if (self->enable_endpoint_discovery_queue !=
        from->enable_endpoint_discovery_queue)
    {
        return DDS_BOOLEAN_FALSE;
    }

    if (self->metatraffic_transport_priority !=
        from->metatraffic_transport_priority)
    {
        return DDS_BOOLEAN_FALSE;
    }
