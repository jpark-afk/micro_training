...
const char *const DDS_PUBLICATION_BUILTIN_TOPIC_TYPE_NAME = "DDS_PublicationBuiltinTopicData";
 ...
 * \param[in] remote_dp   The remote participant containing the publication
 * \param[in] remote_pub  The remote publication to match against
 * 
...
NDDS_RemotePublication_match_local_reader(DDS_DomainParticipant *const self,
               ...
               const struct DDS_RemotePublicationImpl *const remote_pub)
{
 ...
 ******************************************************************************/
RTI_PRIVATE DDS_ReturnCode_t
NDDS_DomainParticipant_create_remote_publication(
        DDS_DomainParticipant *const participant,
        struct DDS_RemoteParticipantImpl *remote_dp,
        const struct DDS_PublicationBuiltinTopicData *const data,
        struct DDS_BuiltinTopicKey_t *rem_publication_key,
        struct DDS_RemotePublicationImpl **rem_publication_out)
{
    struct DDS_DomainParticipantImpl *self =
                        (struct DDS_DomainParticipantImpl *)participant;
    DB_ReturnCode_T dbrc;
    ...
    *rem_publication_out = NULL;
    ...
    if (!DDS_PublicationBuiltinTopicData_set_from(&rem_publication->data, data,
                                                    DDS_BOOLEAN_TRUE, participant))
    {
        DDSC_LOG_OBJECT_COPY(OSAPI_LOGKIND_ERROR,DDSC_LOG_PUBLICATIONDATA_OBJECT)
        if (!DDS_PublicationBuiltinTopicData_finalize_no_dealloc(&rem_publication->data,
                                                                participant))
        {
        }
    ...
    rem_publication->data.key = *rem_publication_key;
    rem_publication->data.participant_key = remote_dp->data.key;
    ...
    if (retval == DDS_RETCODE_OK && rem_publication != NULL)
    {
        *rem_publication_out = rem_publication;
    }
                    ...
                    NDDS_TypePluginKeyKind key_kind)
{
    ...
    if (dbrc == DB_RETCODE_NO_DATA)
    {
         ...
         * rem_publication, if the remote datawriter failed to pass trust
         * checks. In this case, the remote publication is ignore and not
         * added to the database
         */
        retcode = NDDS_DomainParticipant_create_remote_publication(
                                        participant,
                                        rem_participant,
                                        data,
                                        &tmp_key,
    ...
    {
#if OSAPI_ENABLE_LOG
        if (!DDS_PublicationBuiltinTopicData_is_equal(&rem_publication->data, data))
        {
            DDSC_LOG_REMOTE_PUBLICATION_DATA_CHANGED(OSAPI_LOGKIND_WARNING)
        }
#endif
        if (rem_publication->as_entity.state == RTIDDS_ENTITY_STATE_ENABLED)
        {
            retcode = NDDS_RemotePublication_match_local_reader(participant,
                                                                rem_participant,
                                                                rem_publication);
                    ...
                    DDS_Boolean *is_queued)
{
    ...
    *is_queued = DDS_BOOLEAN_FALSE;
    ...
    {
        if (DDS_DiscoveryQueue_is_publication_queue_enabled(
                            &participant->discovery_queue))
        {
            retcode = DDS_DiscoveryQueue_queue_publication(
                                       ...
                                       DDS_Boolean reset_entry)
{
    ...
    {
        if (!DDS_PublicationBuiltinTopicData_finalize_no_dealloc(&rem_publication->data, participant))
        {
         ...
         */
        if (!DDS_DiscoveryQueue_is_publication_empty(&participant->discovery_queue))
        {
