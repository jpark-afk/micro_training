...
const char *const DDS_SUBSCRIPTION_BUILTIN_TOPIC_TYPE_NAME = "DDS_SubscriptionBuiltinTopicData";
 ...
 * \param[in] remote_dp   The remote participant containing the subscription
 * \param[in] remote_pub  The remote subscription to match against
 * 
...
NDDS_RemoteSubscription_match_local_writer(DDS_DomainParticipant *const self,
            const struct DDS_RemoteParticipantImpl *const remote_dp,
            const struct DDS_RemoteSubscriptionImpl *const remote_sub)
{
    struct DDS_DomainParticipantImpl *participant =
                                    (struct DDS_DomainParticipantImpl *)self;
    struct DDS_DataWriterImpl *local_writer;
 ...
 ******************************************************************************/
RTI_PRIVATE DDS_ReturnCode_t
NDDS_DomainParticipant_create_remote_subscription(
        DDS_DomainParticipant *const participant,
        struct DDS_RemoteParticipantImpl *remote_dp,
        const struct DDS_SubscriptionBuiltinTopicData *const data,
        struct DDS_BuiltinTopicKey_t *rem_subscription_key,
        struct DDS_RemoteSubscriptionImpl **rem_subscription_out)
{
    struct DDS_DomainParticipantImpl *self =
                        (struct DDS_DomainParticipantImpl *)participant;
    DB_ReturnCode_T dbrc;
    ...
    *rem_subscription_out = NULL;
    ...
    if (!DDS_SubscriptionBuiltinTopicData_set_from(&rem_subscription->data, data,
                                                    DDS_BOOLEAN_TRUE, participant))
    {
        DDSC_LOG_OBJECT_COPY(OSAPI_LOGKIND_ERROR,DDSC_LOG_SUBSCRIPTIONDATA_OBJECT)
        if  (!DDS_SubscriptionBuiltinTopicData_finalize_no_dealloc(&rem_subscription->data,
                                                                    participant))
        {
        }
     ...
     */
    rem_subscription->data.key = *rem_subscription_key;
    rem_subscription->data.participant_key = remote_dp->data.key;
    ...
    if (NDDS_RemoteEntity_is_enabled((NDDS_RemoteEntity *)remote_dp))
    {
    ...
    if (retval == DDS_RETCODE_OK && rem_subscription != NULL)
    {
        *rem_subscription_out = rem_subscription;
    }
                    ...
                    NDDS_TypePluginKeyKind key_kind)
{
    ...
    if (dbrc == DB_RETCODE_NO_DATA)
    {
        /* If trust is enabled, this call may succeed and return a NULL
         * rem_subscription, if the remote datareader failed to pass trust
         * checks. In this case, the remote subscription is ignore and not
         * added to the database */
        retcode = NDDS_DomainParticipant_create_remote_subscription(
                    participant,
                    rem_participant,
                    data,
                    &tmp_key,
    ...
    {
#if OSAPI_ENABLE_LOG
        if (!DDS_SubscriptionBuiltinTopicData_immutable_is_equal(&rem_subscription->data, data))
        {
            DDSC_LOG_REMOTE_SUBSCRIPTION_DATA_CHANGED(OSAPI_LOGKIND_WARNING)
        }
#endif

#if DDS_FILTERING_ENABLED
        if (DDS_DomainParticipant_is_filtering_enabled(participant)
                && !DDS_ContentFilterProperty_is_equal(&rem_subscription->data.content_filter,
                                                       &data->content_filter))
        {
            DDS_DomainParticipant_filter_property_finalize_shallow_copy(
                                participant,
                                &rem_subscription->data.content_filter);
            retval = DDS_DomainParticipant_filter_property_shallow_copy(
                                participant,
                                &rem_subscription->data.content_filter,
                                &data->content_filter);
#if OSAPI_ENABLE_LOG
            if (!retval)
            {
                /* Even if we fail to update the remote content filter, we
                 * can continue to match with the remote subscription, but we
                 * will not be able to apply writer filtering.
                 */
                DDSC_LOG_UPDATE_REMOTE_CONTENT_FILTER(OSAPI_LOGKIND_WARNING)
            }
#else
        ...
        {
            retcode = NDDS_RemoteSubscription_match_local_writer(participant,
                                                                 rem_participant,
                                                                 rem_subscription);
            if (retcode != DDS_RETCODE_OK)
            {
                    ...
                    DDS_Boolean *is_queued)
{
    ...
    *is_queued = DDS_BOOLEAN_FALSE;

    retcode = NDDS_DomainParticipant_assert_remote_subscription(
                            participant,participant_name,data,key_kind);

    if (DDS_RETCODE_OUT_OF_RESOURCES == retcode)
    {
        if (DDS_DiscoveryQueue_is_subscription_queue_enabled(
                            &participant->discovery_queue))
        {
            retcode = DDS_DiscoveryQueue_queue_subscription(
                        &participant->discovery_queue,data,info);

            if (retcode == DDS_RETCODE_OK)
            {
                *is_queued = DDS_BOOLEAN_TRUE;
            }
#if OSAPI_ENABLE_LOG
            else
            {
                                        ...
                                        DDS_Boolean reset_entry)
{
    ...
    else
    {
        ...
        /* If there are publication discovery messages queued up, try again
         */
        if (!DDS_DiscoveryQueue_is_subscription_empty(&participant->discovery_queue))
        {
            DDS_DiscoveryQueue_assert_queued_subscription(
                                    participant,&participant->discovery_queue);
        }
