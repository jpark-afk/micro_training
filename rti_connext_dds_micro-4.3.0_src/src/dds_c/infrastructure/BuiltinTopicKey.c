...
{
    /* From RTPS:
     * User Writer (with Key)  ......: 0x02
     * Builtin Writer (with Key) ....: 0xc2
     * User Writer (no Key)  ........: 0x03
     * Builtin Writer (no Key) ......: 0xc3
     * Writer (no Key)
     */
    return ((object_id & 0xf) == 0x3) ||
           ((object_id & 0xf) == 0x2) ?DDS_BOOLEAN_TRUE : DDS_BOOLEAN_FALSE;
}
        ...
        const DDS_InstanceHandle_t *instance_handle)
{
    ...
    self->value[0] = guid.prefix.host_id;
    self->value[1] = guid.prefix.app_id ;
    self->value[2] = guid.prefix.instance_id ;
    self->value[3] = guid.object_id;
}
