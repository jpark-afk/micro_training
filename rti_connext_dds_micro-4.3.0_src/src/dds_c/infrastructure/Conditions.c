 ...
 *   is used.
 *
 *   \return - DDS_RETCODE_OK if the condition was successfully initialized
 *             DDS_RETCODE_ERROR if the initialization failed
 *
...
DDS_ConditionImpl_finalize(struct DDS_ConditionImpl *self)
{
    ...
    while (!REDA_CircularList_node_at_head(&self->_waitsets,&wsref->_node))
    {
            ...
            != DDS_RETCODE_OK)
        {
            ...
            rtn = OSAPI_Mutex_give(self->_state_lock);
            IGNORE_RETVAL(rtn);
 ...
 * 
 *   The caller MUST already hold self->_state_lock before calling this
 *   function.
 *
 *   \param[in] self          The condition whose waitsets are to be notified
 *   \param[in] triggered     DDS_BOOLEAN_TRUE to trigger waitsets,
 *                            DDS_BOOLEAN_FALSE to reset them
                                  ...
                                  DDS_WaitSet *ws)
{
...
#endif
        /* DDS_RETCODE_ILLEGAL_OPERATION means already attached */
     ...
     *                 was unlocked.
     * - ERROR      -> either DDS_WaitSetImpl_add_cond_ref or OSAPI_Mutex_take
     *                 failed.
...
{
    DDS_Boolean trigger = DDS_BOOLEAN_FALSE;
                                     ...
                                     DDS_Boolean value)
{
    ...
    DDS_ConditionImpl_notify_waitsets(&self->_parent,
...
{
    DDS_StatusMask retval;

    OSAPI_PRECONDITION_ALWAYS(self == NULL,
                           return DDS_STATUS_MASK_NONE,
                           OSAPI_Log_entry_add_pointer("self",self,RTI_TRUE);)

    /* This read operation is protected with as an abundance of caution. A
     * multicore system with multiple independent bus controllers may cause
     * an unaligned integer to require multiple cycles.
     */
    if (!OSAPI_Mutex_take(self->_parent._state_lock))
    {
        return DDS_STATUS_MASK_NONE;
                                         ...
                                         DDS_StatusMask mask)
{
    ...
    if (self->_enabled_statuses &
                        DDS_StatusCondition_get_entity(self)->current_statuses)
    {
 ...
 * \param[in] triggered       DDS_BOOLEAN_TRUE if the status is being set,
 *                            DDS_BOOLEAN_FALSE if it is being cleared
 ...
 * \param[in] consumed        Controls whether this function should update the
 *                            entity's active statuses. If DDS_BOOLEAN_TRUE, the
 *                            caller has already updated active_statuses.If
 *                            DDS_BOOLEAN_FALSE, this function will update it.
 *
...
/*ci
 * \brief Get the current status changes for an entity.
 *
 * \details
 * This function returns the entity's current_statuses.
 *
 * \param[in] self The status condition associated with the entity.
 *
 * \return The current status mask for the entity.
 */
DDS_StatusMask
DDS_StatusConditionImpl_get_entity_status_changes(
    struct DDS_StatusConditionImpl *self)
{
    DDS_StatusMask current_statuses = DDS_STATUS_MASK_NONE;
    ...
    current_statuses = self->_entity->current_statuses;
