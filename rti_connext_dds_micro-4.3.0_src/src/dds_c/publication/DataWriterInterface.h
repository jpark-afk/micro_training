...
struct DDS_DataWriterInterfaceProperty
{
     ...
     */
    RTI_INT32 max_send_fanout;

    struct DDS_TypePlugin *type_plugin;
};
...
struct DDS_DataWriterRouteProperty
{
    struct NETIORouteProperty _parent;

    /*ci
     * \brief The encapsulation to use on this route
     */
    RTI_UINT16 encapsulation;
};

#define DDS_DataWriterRouteProperty_INITIALIZER \
{\
    {0},\
    0\
}
...
{
    /*ci
     * \brief the destination reader
     */
    struct NETIO_Guid reader;
    /*ci
...
struct DDS_DataWriterInterface
{
    ...
    DB_Index_T enc_index;

    struct DDS_TypePlugin *type_plugin;

    RTI_BOOL return_payload_after_write;
