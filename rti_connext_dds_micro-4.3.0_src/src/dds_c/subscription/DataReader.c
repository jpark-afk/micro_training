...
DDS_DataReader_enable(DDS_Entity *self)
{
    ...
    if (!DDS_Duration_is_infinite(&datareader->deadline.period))
    {
        DDS_DeadlineQosPolicy_get_sample_freq(&datareader->deadline,
                                              &deadline_sample_hz);
    ...
    {
        if (!datareader->config->on_after_enabled((DDS_DataReader*)self,NULL,DDS_BOOLEAN_FALSE))
        {
            retcode = DDS_RETCODE_ERROR;
            goto done;
        }
    ...
    {
        if (!datareader->config->on_after_enabled((DDS_DataRader*)self,NULL,DDS_BOOLEAN_FALSE))
        {
            retcode = DDS_RETCODE_ERROR;
            goto done;
        }
                            ...
                            DDS_Boolean valid_handle)
{
    ...
    else /* !loan */
    {
        ...
        retcode2 = DDSHST_Reader_finish_read_or_take(datareader->_rh,
                     &sample_ptrs_array,&info_array,sample_count,
                     DDS_BOOLEAN_TRUE);
                                        ...
                                        DDS_Boolean take)
{
    ...
    *sample_info = *info_array[0];
    ...
    retcode2 = DDSHST_Reader_finish_read_or_take(datareader->_rh,
            &sample_ptr_array,&info_array, 1, DDS_BOOLEAN_TRUE);
                               ...
                               const void *key_holder)
{
    ...
    if (DDS_TypePlugin_get_key_kind(datareader->type_plugin) == NDDS_TYPEPLUGIN_NO_KEY)
    {
        /* no key, not failure */
        return nil_handle;
    }
                                  ...
                                  const struct DDS_SampleInfo *sample_info)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (is_data_consistent == NULL)
                              || (sample == NULL) || (sample_info == NULL),
                              return DDS_RETCODE_BAD_PARAMETER,
               OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
               OSAPI_Log_entry_add_pointer("is_data_consistent",is_data_consistent,RTI_TRUE);)
    ...
    *is_data_consistent = DDS_BOOLEAN_FALSE;
    ...
    result = DDS_TypePlugin_is_sample_consistent(datareader->type_plugin,
                                                 is_data_consistent,
                                                 sample,
                                                 sample_info);
    if (result)
    ...
    /* DDS_RETCODE_PRECONDITION_NOT_MET means that whether the sample is consistent
     * could not be determined. If any additional retcodes are added
     * in the future, DDS_TypePlugin_is_sample_consistent will need to be changes
     * to support a error reason;
     */
    return DDS_RETCODE_PRECONDITION_NOT_MET;
}
                       ...
                       const struct DDS_DataReaderQos *qos)
{
    ...
    DDS_Boolean qos_changed = DDS_BOOLEAN_FALSE;
#endif /* DDS_FILTERING_ENABLED */
...
#if DDS_FILTERING_ENABLED
    /* Check consistency inside the lock when accessing shared
     * state such as the DomainParticipant QoS.
     */
    participant = DDS_Subscriber_get_participant(self->subscriber);
    ...
    if (!DDS_DataReader_immutable_is_equal(datareader, qos))
    {
    ...
    /* Update mutable QoS policies. If there are no supported mutable QoS policies,
     * then there is no need to update the datareader's copy of the QoS. If there
     * are multiple mutable QoS policies, then there should be a way to revert to
     * the previous QoS updates if one of them fails.
     */
#if DDS_FILTERING_ENABLED
    if (!DDS_ContentFilterQosPolicy_is_equal(datareader->content_filter_qos,
                                             &qos->content_filter))
    {
        if (datareader->content_filter_qos == &DDS_CONTENT_FILTER_DEFAULT)
        {
            OSAPI_Heap_allocate_struct(&datareader->content_filter_qos,
                                       struct DDS_ContentFilterQosPolicy);
            if (datareader->content_filter_qos == NULL)
            {
                retcode = DDS_RETCODE_OUT_OF_RESOURCES;
                goto done;
            }
            if (DDS_ContentFilterQosPolicy_initialize(datareader->content_filter_qos)
                    != DDS_RETCODE_OK)
            {
                goto done;
            }
        }
        if (!DDS_DataReader_update_filter(datareader, &qos->content_filter))
        {
            DDSC_LOG_CHANGE_CONTENT_FILTER(OSAPI_LOGKIND_ERROR)
            goto done;
        }
        qos_changed = DDS_BOOLEAN_TRUE;
    }

    /* If additional mutable QoS policies are added in the future,
     * then they should be checked here and updated if necessary.
     */

    if (qos_changed)
    {
        /* Update discovery based on new QoS */
        if (datareader->config->on_after_enabled)
        {
            if (!datareader->config->on_after_enabled((DDS_DataReader*)self,NULL,DDS_BOOLEAN_TRUE))
            {
...
DDS_DataReader_get_qos(DDS_DataReader *self, struct DDS_DataReaderQos *qos)
{
    ...
    retcode = DDS_DataReader_get_qos_from(self,qos);
                            ...
                            struct DDS_InstanceHandleSeq *publication_handles)
{
    ...
    struct DDS_DataReaderInterface *dr_intf = NULL;
    ...
    OSAPI_PRECONDITION_ALWAYS(
            (datareader == NULL) || (publication_handles == NULL),
            return DDS_RETCODE_BAD_PARAMETER,
            OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
            OSAPI_Log_entry_add_pointer("publication_handles",publication_handles,RTI_TRUE);)

    dr_intf = (struct DDS_DataReaderInterface *)self->dr_intf;
    ...
    dbrc = DB_Database_lock(datareader->config->db);
    if (dbrc != DB_RETCODE_OK)
    {
        return DDS_RETCODE_ERROR;
    }

    if (!DDS_InstanceHandleSeq_set_length(publication_handles, 0))
    {
        goto done;
    }

    dbrc = DB_Table_select_all_default(dr_intf->_parent._btable, &cursor);
    if (dbrc != DB_RETCODE_OK)
    {
        DDSC_LOG_TABLE_SELECT(OSAPI_LOGKIND_ERROR, dr_intf->_parent._btable, dbrc)
        goto done;
    }
    ...
    do
    {
        ...
        if (dbrc == DB_RETCODE_OK)
        {
            if (!DDS_InstanceHandleSeq_ensure_length(publication_handles, length + 1,
                    self->reader_resource_limits.max_remote_writers))
            {
                goto done;
            }

            handle = DDS_InstanceHandleSeq_get_reference(publication_handles, length);
            if (handle == NULL)
            {
                goto done;
            }
            ...
            OSAPI_Memory_copy(handle, &bind_entry->key.peer, RTI_SIZEOF(handle->octet));
            handle->is_valid = DDS_BOOLEAN_TRUE;
    ...
    DB_Cursor_finish(dr_intf->_parent._btable, cursor);
    cursor = NULL;
                ...
                const DDS_InstanceHandle_t *publication_handle)
{
    ...
    DDS_DataWriter *local_writer = NULL;

    OSAPI_PRECONDITION_ALWAYS(
            (self == NULL) || (publication_data == NULL) || (publication_handle == NULL),
            return DDS_RETCODE_BAD_PARAMETER,
            OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
    ...
    /* Verify that self is matched with publication_handle */
    OSAPI_Memory_copy(&bind_key.peer,&publication_handle->octet[0],
                      (RTI_SIZE_T)sizeof(bind_key.peer));

    dbrc = DB_Table_select_match(dr_intf->_parent._btable,
    ...
    if (dbrc != DB_RETCODE_OK)
    {
        DDSC_LOG_TABLE_SELECT(OSAPI_LOGKIND_ERROR,
                              dr_intf->_parent._btable,
                              dbrc)
        goto done;
    }

    /* Check if the publication exists in the remote_publication_table */
    participant = DDS_Subscriber_get_participant(datareader->subscriber);
    DDS_BuiltinTopicKey_from_instance_handle(&dw_key, publication_handle);
    dbrc = DB_Table_select_match(participant->remote_publisher_table,
                                DB_TABLE_DEFAULT_INDEX,
                                (DB_Record_T*)&remote_publication,
                                (DB_Key_T)&dw_key);
    if (dbrc == DB_RETCODE_OK)
    {
        /* Copy data from remote_publication_table */
    ...
    /* Find the local writer */
    object_id = dw_key.value[3];
    dbrc = DB_Table_select_match(participant->local_writer_table,
                                DB_TABLE_DEFAULT_INDEX,
                                (DB_Record_T)&local_writer,
                                (DB_Key_T)&object_id);
    if (dbrc != DB_RETCODE_OK)
    {
        /* It is unexpected that a matched writer would not be in either
         * the remote_publication_table or the local_writer_table.
         */
        DDSC_LOG_TABLE_SELECT(OSAPI_LOGKIND_ERROR,
    ...
    /* Copy data from local writer */
    if (!DDS_PublicationBuiltinTopicData_copy_from_datawriter(publication_data,
                                                              local_writer))
    {
        goto done;
    }
...
DDS_TypeCode*
DDS_DataReader_get_typecode(DDS_DataReader *self)
{
