...
{
    /*ci \brief Peer DataWriter. Must be the first member.
     */
    struct DataReaderBindEntryKey key;

    /*ci
     * \brief Last time when there has been a change in activity not including
     * auto and manual participant messages
     */
    struct OSAPI_SystemTime activity_time;
    ...
    /*ci
     * \brief The lease duration. Used to determine
     *        when a writer can be considered as not alive.
     */
    ...
    /*ci
     * \brief Datawriter liveliness kind
     */
    DDS_LivelinessQosPolicyKind liveliness_kind;
#endif /* DDS_LIVELINESS_CHANNEL_ENABLED */

    /*ci
     * \brief The effective encapsulation of a matched writer. Used to
     *        interpret the keyhash and to calculate a keyhash for the
     *        sample if needed.
     */
