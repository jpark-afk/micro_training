...
DDS_Subscriber_enable(DDS_Entity *self)
{
    ...
    if (!subscriber->entity_factory.autoenable_created_entities)
    {
        goto done;
    }
                                 ...
                                 DDS_StatusMask mask)
{
    ...
    if (DDS_TopicDescription_get_participant(topic_desc) !=
        DDS_Subscriber_get_participant(self))
    {
    ...
    subscriber->config->dr_config->address_resolver = subscriber->config->address_resolver;
    subscriber->config->dr_config->domain_id = subscriber->config->domain_id;
    subscriber->config->dr_config->ext_rtps_intf = subscriber->config->ext_rtps_intf;
    subscriber->config->dr_config->packet_pool = subscriber->config->packet_pool;
    subscriber->config->dr_config->resolved_unicast_seq = subscriber->config->resolved_unicast_seq;
    subscriber->config->dr_config->resolved_multicast_seq = subscriber->config->resolved_multicast_seq;

#if DDS_FILTERING_ENABLED
    subscriber->config->dr_config->filter_plugin = subscriber->config->filter_plugin;
#endif
    ...
    if ((dbrc != DB_RETCODE_OK) || (datareader == NULL))
    {
    ...
    if (ddsrc != DDS_RETCODE_OK)
    {
        ...
        DDSC_LOG_DR_INSTANCE_ASSERTION_FAILED(OSAPI_LOGKIND_ERROR)

        return NULL;
    }

    if (!DDS_DataReaderImpl_initialize(
                datareader,
                subscriber,
                topic_desc,
                qos,
    ...
    if (DDS_TypePlugin_has_create_typed_datareader(datareader->type_plugin))
    {
        wrapper = DDS_TypePlugin_create_typed_datareader(datareader->type_plugin,datareader);
        if (wrapper != NULL)
        {
#ifndef RTI_CERT
            wrapper_created = RTI_TRUE;
#endif
            DDS_Entity_set_wrapper(DDS_DataReader_as_entity(datareader),
                                   wrapper);
        }
        else
        {
    ...
    if (retval == NULL)
    {
        if (wrapper_created && (datareader->type_plugin != NULL) &&
             DDS_TypePlugin_has_create_typed_datareader(datareader->type_plugin))
        {
        ...
        else if (datareader != NULL)
        {
            ...
            (void)DB_Table_delete_record(
                            subscriber->config->local_reader_table,
                            (DB_Record_T)datareader);
            datareader = NULL;
        }
    }
                                 ...
                                 DDS_DataReader *datareader)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (datareader == NULL),
                           return DDS_RETCODE_BAD_PARAMETER,
                           OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
    ...
    retval = DDS_Subscriber_delete_datareader_no_lock(self,datareader);
...
DDS_Subscriber_delete_contained_entities(DDS_Subscriber *self)
{
    ...
    retcode = DDS_Subscriber_delete_contained_entities_no_lock(self);
                                          ...
                                          const struct DDS_DataReaderQos * qos)
{
    ...
    if (!DDS_DataReaderQos_is_equal(self->default_qos, qos))
    {
        if (self->default_qos == &DDS_DATAREADER_QOS_DEFAULT)
        {
            OSAPI_Heap_allocate_struct(&self->default_qos,struct DDS_DataReaderQos);
            if (self->default_qos == NULL)
            {
                retcode = DDS_RETCODE_OUT_OF_RESOURCES;
                goto done;
            }
            DDS_DataReaderQos_initialize(self->default_qos);
        }

        retcode = DDS_DataReaderQos_set_from(self->default_qos,
                                            qos, DDS_BOOLEAN_TRUE,
                                            self->participant);
#if OSAPI_ENABLE_LOG
                       ...
                       const struct DDS_SubscriberQos *qos)
{
    ...
    if (!DDS_PartitionQosPolicy_is_consistent_w_limits(&qos->partition,
                ...
                DDS_Subscriber_get_participant(self))->resource_limits))
    {
    ...
    if  ((self->management.is_anonymous != qos->management.is_anonymous) ||
         ...
         REDA_String_compare(self->subscriber_name,qos->subscriber_name.name))
    {
    ...
    retcode = DDS_Subscriber_set_qos_from(self, qos,self->participant);
...
DDS_Subscriber_get_qos(DDS_Subscriber *self, struct DDS_SubscriberQos *qos)
{
    ...
    retcode = DDS_Subscriber_get_qos_from(self,qos);
#if OSAPI_ENABLE_LOG
    if (retcode != DDS_RETCODE_OK)
    {
...
DDS_DataReader*
DDS_Subscriber_lookup_datareader_by_name(DDS_Subscriber *self,
                                         const char *reader_name)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (reader_name == NULL),
           return NULL,
           OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
           OSAPI_Log_entry_add_pointer("reader_name",reader_name,RTI_TRUE);)
    ...
    do
    {
        dbrc = DB_Cursor_get_next(cursor, (DB_Record_T*)&reader);
        if (dbrc == DB_RETCODE_OK)
        {
            if (!DDS_String_ncmp(reader_name,
                     reader->subscription_name,
                     DDS_ENTITYNAME_QOS_NAME_MAX) &&
                (reader->subscriber == self))
            {
                break;
            }
            reader = NULL;
    ...
    DB_Cursor_finish(subscriber->config->local_reader_table,cursor);
