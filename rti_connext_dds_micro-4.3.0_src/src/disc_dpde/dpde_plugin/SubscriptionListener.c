/*
 * FILE: SubscriptionListener.c - Subscription listener API
 *
                                                        ...
                                                        DDS_DataReader *reader)
{
    ...
    participant = DDS_Publisher_get_participant(dpde_plugin->publisher);
    do
    {
        ...
        {
            DPDE_LOG_TAKE_SUBSCRIPTION_SAMPLE(OSAPI_LOGKIND_ERROR,retcode)
            break;
        }
        ...
        for (i = 0; i < l; ++i)
        {
            ...
            if (info->valid_data)
            {
                DDS_Boolean is_queued = DDS_BOOLEAN_FALSE;

                data->participant_key = data->key;
                data->participant_key.value[3] = RTPS_OBJECT_ID_PARTICIPANT;
                OSAPI_TRACE_DDS("[DPDE] Assert remote subscriber",RTI_FALSE)
                OSAPI_TRACE_STRING("topic",data->topic_name,RTI_TRUE)

                retcode2 =
                    NDDS_DomainParticipant_assert_or_queue_remote_subscription(
                            participant,NULL,data,info,
                            NDDS_TYPEPLUGIN_GUID_KEY,&is_queued);

                if ((retcode2 == DDS_RETCODE_OK) && is_queued)
                {
                    if (l > 1)
                    {
                        REDA_Sequence_replace_with_last(
                                            (struct REDA_Sequence*)&data_seq,i);
                        REDA_Sequence_replace_with_last(
                                            (struct REDA_Sequence*)&info_seq,i);
                        /* Process the current element again */
                        i--;
                        l--;
                    }
                    else
                    {
                        /* last element */
                        if (!REDA_Sequence_set_length((struct REDA_Sequence*)&data_seq,0))
                        {
                            /* This should never happen */
                        }
                        if (!DDS_SampleInfoSeq_set_length(&info_seq,0))
                        {
                            /* This should never happen */
                        }
                    }
                }
                else if (DDS_RETCODE_OK != retcode2)
                {
                    DPDE_LOG_ASSERT_REMOTE_SUBSCRIPTION(OSAPI_LOGKIND_ERROR,
                                                        data->topic_name,
                                                        retcode2)
                }
            }
            else
            {
                        ...
                        || (info->instance_state == DDS_NOT_ALIVE_NO_WRITERS_INSTANCE_STATE))
                {
                    ...
                    retcode2 = NDDS_DomainParticipant_remove_remote_subscription(
                       participant,
                       &key);

                    if (retcode2 != DDS_RETCODE_OK)
                    {
                        DPDE_LOG_REMOVE_REMOTE_SUBSCRIPTION(OSAPI_LOGKIND_WARNING,
                                                            retcode2)
                    }
#endif /* !RTI_CERT */
                }
                ...
                {
                    DPDE_LOG_INVALID_SUBSCRIPTION_SAMPLE(OSAPI_LOGKIND_WARNING)
                }
            }
        }
    ...
    {
        DPDE_LOG_RETURN_SUBSCRIPTION_SAMPLE(OSAPI_LOGKIND_ERROR,retcode)
    }
}
...
/*ce \dref_SubscriptionBuiltinDataReaderListener_InstanceReplacedCallback
 */
RTI_PRIVATE void
DPDE_SubscriptionBuiltinDataReaderListener_on_instance_replaced(
    void *listener_data,
    DDS_DataReader *reader,
