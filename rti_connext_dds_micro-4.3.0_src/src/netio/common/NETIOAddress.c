...
7 * 16sep2014,tk MICRO-913/PR#11266  Added constraint low_index <= high_index
...
typedef union
{
     ...
     */
    NETIO_RangeToken_T range_token;
...
NETIO_Address_set_ipv4(struct NETIO_Address *addr,RTI_UINT32 port,RTI_UINT32 address)
{
    ...
    addr->port = port;
    addr->value.ipv4.address = NETIO_htonl(address);
}

void
NETIO_Address_set_shmem(struct NETIO_Address *addr,RTI_UINT32 port,RTI_UINT32 address)
{
    addr->kind = NETIO_ADDRESS_KIND_SHMEM;
    addr->port = port;
...
{
    return (addr->kind == NETIO_ADDRESS_KIND_INTRA ? RTI_TRUE : RTI_FALSE);
}

RTI_BOOL
NETIO_Address_set_multicast(struct NETIO_Address *const addr)
{
    OSAPI_PRECONDITION_ALWAYS((addr == NULL),
                            return RTI_FALSE,
                            OSAPI_Log_entry_add_pointer("addr",addr,RTI_TRUE);)
...
{
    return addr->kind & 0x7fffffff;
}

RTI_INT32
NETIO_Address_kind(RTI_INT32 kind)
{
    return kind & 0x7fffffff;
}
                      ...
                      const struct NETIO_Address *raddr)
{
    ...
    if (laddr->kind == NETIO_ADDRESS_KIND_SHMEM)
    {
        return 0;
    }
    ...
    {
        RTI_UINT32 left_val =
                NETIO_ntohl(laddr->value.as_uint32.value[i]);

        RTI_UINT32 right_val =
                NETIO_ntohl(raddr->value.as_uint32.value[i]);
                                  ...
                                  RT_ComponentFactoryId_T *id)
{
    ...
    dbrc = DB_Table_select_all(nar->resolver_table,DB_TABLE_DEFAULT_INDEX,&eh);
    dbrc = DB_Cursor_get_next(eh,(DB_Record_T)&r_entry);
    ...
    DB_Cursor_finish(nar->resolver_table,eh);
 ...
 */
MUST_CHECK_RETURN RTI_PRIVATE RTI_BOOL
NETIO_Address_get_token(const char **c_ptr,NETIO_AddressToken_T *token,
