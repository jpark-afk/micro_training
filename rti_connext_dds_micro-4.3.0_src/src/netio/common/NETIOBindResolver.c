 ...
 *
 * Copyright (c) 2012-2025 Real-Time Innovations, Inc.
 *
 * All rights reserved.
 *
                                 ...
                                 const DB_Record_T op1, void *op2)
{
    ...
    for (i = 0; i < NETIO_ADDRESS_MAX_32BIT; ++i)
    {
        RTI_UINT32 left_val =
                NETIO_ntohl(
                        record_left->key.address.value.as_uint32.value[i]);
...
RTI_INT32
NETIO_BindResolver_get_min_mtu_by_kind(NETIO_BindResolver_T *r_table,
                                        NETIO_RouteKind_T kind )
{
    DB_ReturnCode_T dbrc;
    DB_Cursor_T cursor = NULL;
    struct NETIO_RouteEntry *r_entry = NULL;
    RTI_UINT32 min_mtu = INT_MAX; /* Function returns a max of INT32 */
    struct NETIO_TransportProperty p = NETIO_TransportProperty_INITIALIZER;

    dbrc = DB_Table_select_all(r_table->route_table, DB_TABLE_DEFAULT_INDEX,&cursor);
    if (dbrc != DB_RETCODE_OK)
    {
       return -1;
    }
...
/*ci \brief Check if an address is duplicated in the sequence
 *
 *  @param[in] seq - A sequence to check
 *  @param[in] max_len - The maximum number of elements to search. Must be
 *                       less than or equal to the sequence length
 *  @param[in] address - The address to check if already exists in seq
 *
 *  @return Returns RTI_TRUE if the address exists in the sequence <seq>
 *                  RTI_FALSE otherwise.
 */
RTI_PRIVATE RTI_BOOL
NETIO_BindResolver_is_address_duplicate(struct NETIO_AddressSeq *seq,
                                        RTI_INT32 max_len,
                                        struct NETIO_Address *address)
{
    RTI_INT32 dup_index;

    /* Check if the address exists in the sequence, if so return TRUE */
    for (dup_index = 0; dup_index < max_len; ++dup_index)
    {
        struct NETIO_Address *left_addr;

        left_addr = NETIO_AddressSeq_get_reference(seq,dup_index);

        if (NETIO_Address_compare(left_addr,address) == 0)
        {
            return RTI_TRUE;
        }
    }
...
{
/* Only 1 address is resolved at a time */
#define MAX_CANDIDATE_ARRAY (1)

/* The maximum number of locators that can be reserved in total in a single
 * call to this function are 4 unicast and 4 multicast addresses.
 */
#define MAX_RESERVED_ARRAY  (8)

    NETIO_Interface_T *netio_intf = NULL;
    RTI_INT32 i,k;
    RTI_BOOL retval = RTI_FALSE;
    RTI_INT32 mc_len, max_mc_len;
    RTI_INT32 uc_len, max_uc_len;
    RTI_INT32 orig_mc_len;
    RTI_INT32 orig_uc_len;
    RTI_INT32 last_resvd_len,resvd_len;
    RT_ComponentFactoryId_T id;
    NETIO_AddressResolveContext_T context;
    RTI_BOOL parse_error;
    struct NETIO_Address address = NETIO_Address_INITIALIZER;
    RTI_BOOL route_added;
    struct NETIO_Address candidate_array[MAX_CANDIDATE_ARRAY];
    struct NETIO_Address reserved_array[MAX_RESERVED_ARRAY];
    ...
    OSAPI_Memory_zero(candidate_array,(RTI_SIZE_T)sizeof(candidate_array));
    OSAPI_Memory_zero(reserved_array,(RTI_SIZE_T)sizeof(reserved_array));

    /* The loan is local to this function to local sequences, do direct loan
     * since the loan APIs are not available in CERT.
     */
    candidate_address._contiguous_buffer = candidate_array;
    candidate_address._maximum = MAX_CANDIDATE_ARRAY;

    reserved_address._contiguous_buffer = reserved_array;
    reserved_address._maximum = MAX_RESERVED_ARRAY;
    ...
    /* The maximum number of addresses that can be added to each
     * sequence.
     */
    max_mc_len = NETIO_AddressSeq_get_maximum(mc_locator_seq) -
                        NETIO_AddressSeq_get_length(mc_locator_seq);

    max_uc_len = NETIO_AddressSeq_get_maximum(uc_locator_seq) -
                        NETIO_AddressSeq_get_length(uc_locator_seq);

    if ((max_mc_len == 0) && (max_uc_len == 0))
    {
        return RTI_TRUE;
    }

    if (!NETIO_AddressSeq_set_length(&candidate_address,1))
    {
        NETIO_LOG_BIND_SET_MAX(OSAPI_LOGKIND_ERROR,1)
        return RTI_FALSE;
    }
    ...
    for (i = 0; i < REDA_StringSeq_get_length(locators); ++i)
    {
        ...
        route_added = RTI_FALSE;

        /* Loop through all resolved addresses regardless of whether they
         * can be reserved or not. This ensures the any open cursors is
         * finished.
         */
        while (NETIO_AddressResolver_get_next(r_table->nar,&netio_intf,
                                              ...
                                              &parse_error))
        {
            if (NETIO_BindResolver_is_address_duplicate(mc_locator_seq,
                                                        orig_mc_len,
                                                        &address))
            {
                /* Consider the address added to avoid return error */
                route_added = RTI_TRUE;
            }
            else if (NETIO_BindResolver_is_address_duplicate(
                        uc_locator_seq,orig_uc_len,&address))
            {
                /* Consider the address added to avoid return error */
                route_added = RTI_TRUE;
            }
                        ...
                        &reserved_address,
                        NETIO_AddressSeq_get_length(&reserved_address),
                        &address))
            {
                /* Consider the address added to avoid return error */
                route_added = RTI_TRUE;
             ...
             */
            if ((max_mc_len == 0) && NETIO_Address_is_multicast(&address))
            {
             ...
             * ignore the address and do not add it as a candidate
             */
            if ((max_uc_len == 0) && !NETIO_Address_is_multicast(&address))
            {
            ...
            *NETIO_AddressSeq_get_reference(&candidate_address,0) = address;

            /* Start at the end of the reserved sequence when adding addresses,
             * offset the maximum from the current length since max_uc_len and
             * max_mc_len are the remaining max, not absolute max.
             */
            last_resvd_len = NETIO_AddressSeq_get_length(&reserved_address);

            /* Since an address must be either unicast or multicast
             * set the maximum to the current maximum left for the address
             * type. This ensures the number of returned reserved addresses
             * will fit in the return sequences.
             */
            if (NETIO_Address_is_multicast(&address))
            {
                reserved_address._maximum = max_mc_len + last_resvd_len;
            }
            else
            {
                reserved_address._maximum = max_uc_len + last_resvd_len;
            }
            ...
            {
                /* Failed to reserve the locator even if it could be resolved.
                 */
                continue;
            }

            /* A route was successfully added. Note that in case the call
             * resulted in no new addresses reserved it means they were
             * duplicates of already reserved addresses. This is ok and
             * considered routes being added.
             */
            route_added = RTI_TRUE;

            if (NETIO_Address_is_internal(&address))
            {
                /* Internal locators may be reserved, but they are not
                 * returned to be announced.
                 */
                continue;
            }

            /* The address could be resolved and reserved, add it */
            resvd_len = NETIO_AddressSeq_get_length(&reserved_address);
            for (k = last_resvd_len; k < resvd_len; ++k)
            {
                struct NETIO_Address *an_address;
                RTI_BOOL bool_rc;

                an_address = NETIO_AddressSeq_get_reference(&reserved_address,k);

                if (!NETIO_BindResolver_add_route(r_table,kind,&id,
                                                  an_address,netio_intf,NULL))
                {
                   /* A reserved address could not be added to be listened to.
                    * This is an error, release the remaining reserved
                    * addresses since release_addresses also deletes routes.
                    */
                    RTI_INT32 rem_index;

                    for (rem_index = k; rem_index < resvd_len; rem_index++)
                    {
                        struct NETIO_Address *rel_address;
                        rel_address = NETIO_AddressSeq_get_reference(
                                            &reserved_address,rem_index);
                        bool_rc = NETIO_Interface_release_address(netio_intf,
                                                                  rel_address);
#if OSAPI_ENABLE_LOG
                        if (!bool_rc)
                        {
                            /* Try to release everything even if there is
                             * failure
                             */
                            OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)
                        }
#else
                        IGNORE_RETVAL(bool_rc);
#endif
                    }

                    /* reset the length to the remaining addresses. The
                     * return value can be ignored because the function
                     * has already failed and will return FALSE.
                     */
                    bool_rc = NETIO_AddressSeq_set_length(&reserved_address,k);
                    IGNORE_RETVAL(bool_rc);

                    /* release and delete routes for all previously reserved
                     * addresses.
                     */
                    goto done;
                }

                if (NETIO_Address_is_multicast(an_address))
                {
        ...
        if (!route_added)
        {
            /* If no transport was able to resolve and reserve the address,
    ...
    if (!retval)
    {
        ...
        /* If there is a failure to release addresses from this call
         * report failure. It is unknown why there was a failure. The
         * addresses will not be reported back and addresses already
         * reserved prior to this call is released. Since the function
         * has already failed, ignore the return values.
         *
         * The caller may try to reserve addresses again.
         */
        rc = NETIO_BindResolver_release_addresses(r_table,
                          transports,kind,&reserved_address);
        IGNORE_RETVAL(rc);
        OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)

        rc = NETIO_AddressSeq_set_length(mc_locator_seq,orig_mc_len);
        IGNORE_RETVAL(rc);
        OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)

        rc = NETIO_AddressSeq_set_length(uc_locator_seq,orig_uc_len);
        IGNORE_RETVAL(rc);
        OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)
    }
                                      ...
                                      struct NETIO_AddressSeq *locator_seq)
{
    ...
    for (i = 0; i < len; ++i)
    {
        ...
        for (j = 0; j < REDA_StringSeq_get_length(transport); ++j)
        {
            ...
            if (NETIO_BindResolver_lookup_by_address(r_table,kind,&id,
                                                     &netio_address,
                                                     &netio_intf))
            {
                /* A reserved address is guaranteed to be released by
                 * the transport that resolved and reserved it. Thus, it is
                 * a failure if the address cannot be released.
                 */
                if (!NETIO_Interface_release_address(netio_intf,
                                                     &netio_address))
                {
                    OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)
                    goto done;
                ...
                if (!NETIO_BindResolver_delete_route(r_table,kind,
                                                         &id,&netio_address,
                                                         NULL))
                {
                    OSAPI_LOG_LAST_RECORDED_ERROR(OSAPI_LOGKIND_WARNING)
                ...
                /* The address was released. Continue to the next address */
                break;
            }
        }

        if (j == REDA_StringSeq_get_length(transport))
        {
            /* An address could not be resolved that was previously reserved.
             * Since the lookup also used this route table, it must be the same
             * interface.
             */
            goto done;
        }
