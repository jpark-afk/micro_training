                          ...
                          RTI_BOOL copy_content)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (new_max <= 0) ||
                           !REDA_Sequence_has_ownership(self) ||
                           (self->_element_size <= 0),
                           return RTI_FALSE,
                           OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
                           OSAPI_Log_entry_add_int("new_max",new_max,RTI_FALSE);
                           OSAPI_Log_entry_add_int("ownership",
                            REDA_Sequence_has_ownership(self),RTI_FALSE);
                           OSAPI_Log_entry_add_int("element_size",
                                        self == NULL ? 0 : self->_element_size,
                                        RTI_TRUE);)
    ...
    if ((RTI_UINT32)new_max > (UINT_MAX/(RTI_UINT32)self->_element_size))
    {
        REDA_LOG_SEQUENCE_ALLOC_FAILED(OSAPI_LOGKIND_ERROR,UINT_MAX,
                                        OSAPI_ALIGNMENT_DEFAULT)
        return RTI_FALSE;
    }

    OSAPI_Heap_allocate_buffer((char **)&new_buffer,
                        (RTI_UINT32)new_max * (RTI_UINT32)self->_element_size,
                        OSAPI_ALIGNMENT_DEFAULT);
    if (new_buffer == NULL)
    {
        REDA_LOG_SEQUENCE_ALLOC_FAILED(
            OSAPI_LOGKIND_ERROR,(RTI_UINT32)new_max * (RTI_UINT32)self->_element_size,
            OSAPI_ALIGNMENT_DEFAULT)
        return RTI_FALSE;
    }
                          ...
                          RTI_BOOL copy_content)
{
    ...
    /* If the sequence element is contiguous in memory, copy_content
    ...
    if (copy_content && (new_length > 0))
    {
        ...
        OSAPI_Memory_copy(new_buffer,
                        ...
                        (RTI_UINT32)new_length * (RTI_UINT32)self->_element_size);
    }
...
{
#ifdef RTI_CERT
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (self->_contiguous_buffer == NULL),
            return NULL,
            OSAPI_Log_entry_add_pointer("self",self,RTI_FALSE);
            OSAPI_Log_entry_add_pointer("self->_contiguous",
                       (self == NULL)?NULL:self->_contiguous_buffer,RTI_TRUE);)
#else
                   ...
                   RTI_BOOL copy_content)
{
    ...
    RTI_INT32 src_max = 0;
#ifdef RTI_CERT
    OSAPI_PRECONDITION_ALWAYS((self == NULL) || (src == NULL) ||
                       ...
                       RTI_BOOL compare_content)
{
    ...
    OSAPI_PRECONDITION_ALWAYS((left == NULL) || (right == NULL) ||
            (left->_element_size == 0) || (right->_element_size == 0),
            return RTI_FALSE,
            OSAPI_Log_entry_add_pointer("left",left,RTI_FALSE);
            OSAPI_Log_entry_add_pointer("right",right,RTI_FALSE);
            OSAPI_Log_entry_add_int("left->_element_size",
                    (left==NULL)?0:left->_element_size,RTI_FALSE);
            OSAPI_Log_entry_add_int("right->_element_size",
                    (right==NULL)?0:right->_element_size,RTI_TRUE););
#else
    OSAPI_PRECONDITION_ALWAYS((left == NULL) || (right == NULL),
            return RTI_FALSE,
            OSAPI_Log_entry_add_pointer("left",left,RTI_FALSE);
...
{
    /* return the i-th pointer in array of pointers */
    return &((void **)(self->_contiguous_buffer))[i];
}

void
REDA_Sequence_replace_with_last(struct REDA_Sequence *self,RTI_INT32 i)
{
    if (self->_flags & REDA_SEQUENCE_FLAG_DISCONTIGUOUS)
    {
        void **ref_ptr = REDA_Sequence_get_address(self,i);

        *ref_ptr = REDA_Sequence_get_reference(self,self->_length - 1);
        ref_ptr = REDA_Sequence_get_address(self,self->_length - 1);
        *ref_ptr = NULL;
    }
    else
    {
        void *ref_dst = REDA_Sequence_get_reference(self,i);
        void *ref_src = REDA_Sequence_get_reference(self,self->_length - 1);
        OSAPI_Memory_copy(ref_dst,ref_src,(RTI_SIZE_T)self->_element_size);
    }
    self->_length--;
}
