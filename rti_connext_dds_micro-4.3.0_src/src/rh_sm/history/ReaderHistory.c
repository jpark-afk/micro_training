                            ...
                            struct RHSM_HistoryKeyEntry *key)
{
    ...
    if (self->owner_length > 0)
    {
        OSAPI_Memory_zero(key->owner_array,
                          ...
                          RTI_SIZEOF(struct RHSM_HistoryRWEntry *));
    }
    else
    {
        OSAPI_Memory_zero(key->owner_bitmap,
                          MAX_OWNER_BITMAP_LENGTH *
                          MAX_OWNER_BITMAP_ELEMENT_SIZE);
    }
...
/*ci
 * \brief Return true if key has already been deleted.
 *
 * \param[in] key  The key to delete
 *
 * \return RTI_TRUE if deleted, RTI_FALSE if not.
 */
RTI_PRIVATE RTI_BOOL
RHSM_History_is_key_deleted(struct RHSM_HistoryKeyEntry *key)
{
    return ((key->_samples._next == NULL) &&
            (key->_samples._prev == NULL) &&
             ((key->_instance_state == DDS_NOT_ALIVE_DISPOSED_INSTANCE_STATE) ||
              (key->_instance_state == DDS_NOT_ALIVE_NO_WRITERS_INSTANCE_STATE)));
}
...
RHSM_History_delete_key(struct RHSM_History *self,
                        struct RHSM_HistoryKeyEntry *key)
{
    if (RHSM_History_is_key_deleted(key))
    {
        /* Already deleted. THis can happen if a key is deleted while
         * in a listener and the key is again deleted on return.
         */
        return RTI_TRUE;
    }
    ...
    if (RHSM_History_is_deadline_enabled(self))
    {
        REDA_CircularList_unlink_node(&key->_deadline_entry);
    }

    if (RHSM_History_is_replace_oldest_instance(self))
    {
    ...
    /* Indicates a deleted key, do not attempt to delete this key again */
    key->_samples._next = NULL;
    key->_samples._prev = NULL;
...
{
    RHSM_History_set_in_listener(self);
    ...
    RHSM_History_clear_in_listener(self);
}
                           ...
                           RTI_BOOL *new_samples)
{
    ...
    if (sample->_info.valid_data)
    {
         ...
         */
        if (key->_sample_count == (self->max_samples_per_instance + 1))
        {
            ...
            else if (RHSM_History_is_keep_all(self) &&
                     RHSM_History_is_reliable(self))
            {
                /* This means that the reader cache has not taken samples
                 * from the cache to make room for new samples or that it
                 * is best_effort. In this case we still try to remove it.
                 */
                sample_rejected = RTI_TRUE;
            }
            else
            {
                /* Must be KEEP_LAST or KEEP_ALL and BEST_EFFORT. This
                 * scenario can happen if all the samples in the current
                 * history are "read()" from the cache and still on loan.
                 */
                ...
                if (!(sample->_flags & RHSM_HISTORY_FLAG_RECOMMITTED)
                    && RHSM_History_is_keep_last(self))
                {
        ...
        /* In the case of KEEP_ALL, depth is ignored */
        if (RHSM_History_is_keep_last(self) &&
            (key->_history_depth == self->history_depth))
        {
    ...
    if (sample->_info.valid_data)
    {
        ...
        if (key->_history_depth >= self->history_depth)
        {
                             ...
                             struct OSAPI_SystemTime *now)
{
    ...
    sample_info->source_timestamp.sec = now->sec;
    sample_info->source_timestamp.nanosec = now->nanosec;
...
RHSM_History_writer_limit_reached(struct RHSM_History *self,
                                  struct RHSM_HistoryKeyEntry *key_entry)
{
    return ((key_entry->_writer_count == self->max_remote_writers_per_instance)
            ? RTI_TRUE : RTI_FALSE);
}
                      ...
                      struct RHSM_HistoryRWEntry *rw_entry)
{
    ...
    if (!OSAPI_System_get_time(&now))
    {
         ...
         */
        now.sec = OSAPI_SYSTEM_TIME_SEC_MAX;
        now.nanosec = OSAPI_SYSTEM_TIME_NANO_MAX;
                    ...
                    const struct DDSHST_ReaderListener *const listener)
{
    ...
    if (property->_parent.destination_order.kind != DDS_BY_RECEPTION_TIMESTAMP_DESTINATIONORDER_QOS)
    {
    ...
    if ((property->_parent.reader_resource_limits.instance_replacement !=
             DDS_NO_INSTANCE_REPLACEMENT_QOS) &&
        ((property->_parent.reader_resource_limits.instance_replacement !=
    ...
    if (property->_parent.history.kind == DDS_KEEP_ALL_HISTORY_QOS)
    {
        RHSM_History_set_keep_all(history);
    }

    if (property->_parent.durability.kind == DDS_VOLATILE_DURABILITY_QOS)
    {
        RHSM_History_set_volatile_durability(history);
    }
    else if (property->_parent.durability.kind == DDS_TRANSIENT_LOCAL_DURABILITY_QOS)
    {
        RHSM_History_set_transient_local_durability(history);
    }
    else
    {
        goto done;
    ...
    if (property->_parent.reliability.kind == DDS_RELIABLE_RELIABILITY_QOS)
    {
        RHSM_History_set_reliable(history);
    }
    history->max_blocking_time = property->_parent.reliability.max_blocking_time;

    if (property->_parent.reader_resource_limits.instance_replacement == DDS_REPLACE_OLDEST_INSTANCE_REPLACEMENT_QOS)
    {
        RHSM_History_set_replace_oldest_instance(history);
    }
    history->max_remote_writers_per_instance = property->_parent.reader_resource_limits.max_remote_writers_per_instance;
    if (property->_parent.ownership.kind == DDS_EXCLUSIVE_OWNERSHIP_QOS)
    {
        RHSM_History_set_ownership_exclusive(history);
    }
    history->db = property->_parent._parent.db;
    history->timer = property->_parent._parent.timer;
     ...
     */
    if (property->_parent.reader_resource_limits.max_remote_writers >
                           (RTI_INT32)(RHSM_RW_PER_ELEMENT *
                                       MAX_OWNER_BITMAP_LENGTH))
    {
        history->owner_length = property->_parent.reader_resource_limits.max_remote_writers_per_instance;
    }
    else
    {
    ...
    pool_property.buffer_size = RTI_SIZEOF(struct RHSM_HistoryRWEntry);
    pool_property.max_buffers =
            (RTI_SIZE_T)property->_parent.reader_resource_limits.max_remote_writers;

    rw_init.current_index = 0;
    rw_init.max_rw = property->_parent.reader_resource_limits.max_remote_writers;
    ...
    index_prop.max_entries =
                property->_parent.reader_resource_limits.max_remote_writers;
    ...
    pool_property.buffer_size = RTI_SIZEOF(struct RHSM_HistoryKeyEntry);
    pool_property.max_buffers = (RTI_SIZE_T)property->_parent.resource_limits.max_instances;
#ifndef RTI_CERT
    ...
    index_prop.max_entries = property->_parent.resource_limits.max_instances;
    ...
    if (RHSM_History_is_keep_all(history))
    {
        history->history_depth = history->max_samples_per_instance;
    }
    else
    {
        history->history_depth = property->_parent.history.depth;
    }
     ...
     */
    pool_property.buffer_size = RTI_SIZEOF(struct RHSM_HistorySample);
    pool_property.max_buffers = (RTI_SIZE_T)property->_parent.resource_limits.max_samples;
     ...
     * 1 extra for meta samples. A precondition is that max_instances is finite.
     */
    read_seq_max = (property->_parent.resource_limits.max_instances * (history->history_depth + 1));

    /* allocate internal sample and sample_info sequences */
    pool_property.max_buffers = (RTI_SIZE_T)property->_parent.reader_resource_limits.max_outstanding_reads;

    /* buffers of loaned sample infos */
    pool_property.buffer_size = RTI_SIZEOF(struct DDS_SampleInfo*) *
                                (RTI_SIZE_T)read_seq_max;
    ...
    pool_property.buffer_size = (RTI_SIZEOF(void *) *
                                (RTI_SIZE_T)read_seq_max);
    ...
    {
        RHSM_History_set_deadline_enabled(history);
    }
#endif
 ...
 *
 * \return RTI_TRUE if a sample was successfully removed, otherwise RTI_FALSE
 */
MUST_CHECK_RETURN RTI_PRIVATE RTI_BOOL
                           ...
                           DDS_SampleRejectedStatusKind *reject_reason)
{
    ...
    if (RHSM_History_is_keep_all(self))
    {
        if (RHSM_History_is_best_effort(self))
        {
    ...
    while (!done)
    {
        ...
        if ((self->reserved_entry.sample == NULL) && prune_sample)
        {
            if (RHSM_History_prune_sample(self,key))
            {
                self->reserved_entry.sample = REDA_BufferPool_get_buffer(self->_sample_pool);
            }
        }
        ...
        if (RHSM_History_is_reliable(self) ||
            RHSM_History_is_keep_all(self) ||
        ...
        if (!is_suspended)
        {
            ...
            if (!DDS_Duration_is_infinite(&self->max_blocking_time))
            {
                max_blocking_time.sec = self->max_blocking_time.sec;
                max_blocking_time.nanosec = self->max_blocking_time.nanosec;
            }
            else
            {
		...
		/* Upon entering this code the lock is taken exactly once
         * (from the downstream transport. Thus, a single unlock is sufficient.
		 */
        if (DB_Database_unlock(self->db) != DB_RETCODE_OK)
        {
         ...
         * in case we're blocked. That will be symmetric to the
         * sending side.
         */
        brc = OSAPI_Semaphore_take_sec_nanosec(suspend_state.wakeup_sem,
        ...
        /* Check the return code here, when the lock has been retaken */
        if (brc)
        {
...
#else /* OSAPI_THREAD_SEMAPHORE_ENABLED */
        done = RTI_TRUE;
#endif
    } /* end while */
...
RHSM_History_writer_is_owner(struct RHSM_History *self,
                             ...
                             struct RHSM_HistoryRWEntry *rw_entry)
{
    if (RHSM_History_is_ownership_shared(self))
    {
                        ...
                        DDS_SampleRejectedStatusKind *reason)
{
    ...
    *reason = DDS_NOT_REJECTED;
    ...
    if ((key_entry == NULL) && RHSM_History_is_replace_no_instance(self))
    {
                       ...
                       DDS_Long *missing_vsn)
{
    ...
    struct OSAPI_SystemTime rcv_timestamp = OSAPI_TIME_ZERO;
    struct RHSM_HistoryKeyEntry *key_entry = NULL;
    DDS_SampleRejectedStatusKind reason;
    ...
    while (!done)
    {
        ...
        while (!REDA_CircularList_node_at_head(&rw_entry->_cached,&s1_entry->_node))
        {
            ...
            OSAPI_Memory_copy(&rw_entry->last_virtual_sn,
                              &s1_entry->_info.publication_virtual_sequence_number,
                              RTI_SIZEOF(struct REDA_SequenceNumber));
            ...
            s1_entry->_info.reception_timestamp.sec = rcv_timestamp.sec;
            s1_entry->_info.reception_timestamp.nanosec = rcv_timestamp.nanosec;
            ...
            if (!RHSM_History_commit_sample(self,s1_entry,&new_samples))
            {
                 ...
                 */
                RHSM_History_set_recommit_samples(self);
        ...
        RHSM_History_clear_samples_returned(self);
         ...
         * then try again.
         */
        if (!(RHSM_History_is_recommit_samples(self)
              && RHSM_History_is_samples_returned(self)))
        {
            done = RTI_TRUE;
        }
    }
    ...
    *info_array = (struct DDS_SampleInfo **)
    ...
    while ((entry != last_entry) && (*sample_count < (max_samples)))
    {
             ...
             (entry->_key_entry->_view_state & view_states))
        {
            ...
            {
                /* Check if the sample has a sample accessor interface */
                if (entry->_sample->sample_access_intf == NULL)
                {
                    (*sample_ptr_array)[*sample_count] = entry->_sample->holder->sample;
                }
                else
                {   /*Use the sample accesor to get the data*/
                    if (!NETIO_SampleI_retrieve(entry->_sample->sample_access_intf,
                                            entry->_sample->holder->sample,
                                            &(*sample_ptr_array)[*sample_count]))
                    {
                        /* The sample accessor has failed we can not proceed.
                         * This is possibly a catastrophic error. If we are in the
                         * middle of processing samples we just return the one's we have processed
                         * The user maybe unaware of this problem until the next time they call read/take
                         */
                        break;
                    }
                    if ((*sample_ptr_array)[*sample_count] == NULL)
                    {
                        /* This specific sample may have been overwritten. We return the entry
                         * back to the cache and jump to the next entry
                         */
                        RHSM_History_return_sample(self,
                                    entry,DDS_SAMPLE_LOST_BY_MANAGEDPOOL_OVERWRITE);
                        entry = next_entry;
                        continue;
                    }
                }
...
RHSM_History_on_recommit_event(struct DDSHST_Reader *rh)
{
     ...
     */
    RHSM_History_clear_recommit_samples(self);
    while (rw_entry != NULL)
    {
        rw_size = rw_entry->_size;

        saved_notify = rw_entry->_notify_on_commit;
        rw_entry->_notify_on_commit = DDS_BOOLEAN_FALSE;

        if (RHSM_History_commit_rw(rh,
                   rw_entry,
                   &rw_entry->highest_noncommmitable_sn,
                   rw_entry->_strength,
                   &vsn,&mvsn) != DDSHST_RETCODE_SUCCESS)
        {
            RHSM_LOG_RECOMMIT_FAILURE(OSAPI_LOGKIND_ERROR)
        }

        rw_entry->_notify_on_commit = saved_notify;
                                 ...
                                 DDS_Boolean is_loaned_array)
{
    ...
    for (i = 0; i < sample_count; ++i)
    {
        ...
        /*If the samples have a sample accessor interface we need to return the memory back*/
        if ((entry->_sample != NULL) && (entry->_sample->sample_access_intf != NULL))
        {
            NETIO_SampleI_return(entry->_sample->sample_access_intf,
                                entry->_sample->holder->sample,
                                (*sample_ptr_array)[i]);
        }
              ...
              RHSM_History_is_keep_last(self)))
        {
            ...
            {
                RHSM_History_set_samples_returned(self);
            }
        }
    ...
    if (RHSM_History_is_recommit_samples(self) && !RHSM_History_is_in_listener(self))
    {
