...
#if defined(RTI_QNX6) && !defined(RTI_QNX8)
/* This prevents inline functions for byte-swapping from being
 * included, as these are not used by RTI.
 * These workarounds are specific to QNX 6/7 io-pkt networking stack
 * and must not be used on QNX 8 which uses a NetBSD-based io-sock2 stack.
 */
...
/*ci
 * \brief MICRO-12928: Default maximum number of AUTOSAR UDP interface
 *        instances that can be registered for socket recovery.
 *        Can be overridden at compile time.
 */
#ifndef UDP_INTERFACE_AUTOSAR_MAX_INSTANCES
#define UDP_INTERFACE_AUTOSAR_MAX_INSTANCES 4
#endif

/*ci
 * \brief MICRO-12928: Dynamically allocated array of pointers to all
 *        AUTOSAR UDP interface instances, used by
 *        UDP_Interface_recover_sockets() to iterate over all interfaces
 *        during NoCom/FullCom socket recovery.
 */
RTI_PRIVATE struct UDP_Interface **UDP_Interface_fv_AutosarInstances = NULL;

/*ci
 * \brief MICRO-12928: Current number of registered AUTOSAR UDP interface
 *        instances.
 */
RTI_PRIVATE RTI_UINT32 UDP_Interface_fv_AutosarInstanceCount = 0;
...
UDP_Interface_create_send_socket(const struct UDP_Interface *udp_intf, RTI_SEND_SOCKET *sock)
{
...
#if defined(RTI_AUTOSAR)
    /* check if IP is available  and set to invalid*/
    if (!NETIO_Autosar_is_ip_assigned(OSAPI_System_gv_PortProperty->send_local_addr_id))
    {
    ...
    /* MICRO-12928: Explicitly bind the send socket to an ephemeral port.
     * This was previously done lazily inside sendto(), but is now done
     * here so each interface binds its own send socket independently.
     */
    /* move this to UDP_Interface_create_send_socket, do not bind if IP is not available*/
    if (NETIO_AutosarSocket_bind_send(&sock->sock, TCPIP_LOCALADDRID_ANY) != E_OK)
    {
        UDP_LOG_SOCKET_BIND(OSAPI_LOGKIND_ERROR, 0, 0, 0)
        return RTI_FALSE;
    }
#endif /* RTI_AUTOSAR && !RTI_CERT */
                                ...
                                RTI_BOOL *existed)
{
    ...
    int rc;
#if defined(RTI_AUTOSAR)
    RTI_BOOL skip_socket_operation = RTI_FALSE;
    TcpIp_LocalAddrIdType receive_local_addr_id;
    Std_ReturnType ret;
    ...
    sock_address.sin_addr.s_addr = src_addr->value.ipv4.address;
    ret = NETIO_AutosarSocket_find_local_addr_id((struct sockaddr *)&sock_address, &receive_local_addr_id);
    if (ret != E_OK)
    {
        OSAPI_TRACE_NET("find local addr id failed:",RTI_TRUE)
        sock = UDP_INVALID_SOCKET;
        skip_socket_operation = RTI_TRUE;
    }
    else if(!NETIO_Autosar_is_ip_assigned(receive_local_addr_id))
    {
        OSAPI_TRACE_NET("no ip assigned, cannot bind:",RTI_TRUE)
        sock = UDP_INVALID_SOCKET;
        skip_socket_operation = RTI_TRUE;
    }
    else
...
#if defined(RTI_AUTOSAR)
    if(!skip_socket_operation)
    {
#endif /* RTI_AUTOSAR & skip_socket_operation */
#if NETIO_CONFIG_ENABLE_MULTICAST
...
#endif
#if defined(RTI_AUTOSAR) && !defined(RTI_CERT)
    /* if no ip, is_open to false*/
    port_entry->is_open = skip_socket_operation ? RTI_FALSE : RTI_TRUE;
#endif
...
#if defined(__QNXNTO__) && !defined(RTI_QNX8)
            /* QNX 6/7 io-pkt uses short for ifc_len */
...
#if defined(RTI_VXWORKS) || defined(RTI_QNX8)
                            ...
                            RTI_BOOL *existed)
{
...
#elif defined RTI_AUTOSAR
    if (port_entry->_sock != UDP_INVALID_SOCKET)
    {
        return (netiosock_recvnotify(port_entry->_sock,
    ...
    {
        /* Socket is invalid, tcpip stack not ready yet */
        return RTI_TRUE;
    }
#else
...
/* ------------------------------------------------------------------------ */
/*ci
 * MICRO-12928: UDP receive socket recovery for NoCom/FullCom transition.
 * Non-Cert, RTI_AUTOSAR, socket-owner mode only.
 */
#ifndef RTI_CERT
#if defined(RTI_AUTOSAR)
/*ci * \brief MICRO-12928: Re-open and re-bind all UDP sockets on a single UDP
 *        interface after a NoCom/FullCom lifecycle transition.
 *
 * Steps:
 *  1. Refresh the shared send socket (old ID invalidated by TcpIp).
 *     The new socket is bound to a fresh ephemeral port via
 *     NETIO_AutosarSocket_bind_send() (TCPIP_PORT_ANY).  Ephemeral
 *     ports are not preserved across recovery cycles.
 *  2. For each receive UDPPortEntry:
 *     a. Deregister old socket from the recvnotify table.
 *     b. Acquire a new socket ID from TcpIp.
 *     c. Bind the new socket to the same local port.
 *     d. Re-register the receive callback with the new socket ID.
 *     e. Store the new socket ID in the port entry.
 *
 * \param[in] udp_intf  UDP interface whose sockets need recovery.
 *
 * \return RTI_TRUE on success, RTI_FALSE if any socket operation fails.
 */
RTI_PRIVATE RTI_BOOL
UDP_Interface_recover_intf(struct UDP_Interface *udp_intf,
                           TcpIp_LocalAddrIdType local_addr_id)
{
    ...
    /* Step 1: Rebind send sockets */
    for (RTI_UINT32 i = 0 ; i < udp_intf->factory->property->max_unicast_send_sockets; i++)
    {
        rc = NETIO_AutosarSocket_bind_send(&udp_intf->unicast_send_sockets[i].sock, local_addr_id);
        if (rc != 0)
        {
            UDP_LOG_SOCKET_BIND(OSAPI_LOGKIND_ERROR, 0, 0, 0)
            return RTI_FALSE;
    ...
    /* Step 2: Iterate receive port entries and rebind. */
    dbrc = DB_Table_select_all_default(udp_intf->rx_thread_table, &cursor);
    if (dbrc != DB_RETCODE_OK)
    {
        UDP_LOG_PORT_ENTRY(OSAPI_LOGKIND_ERROR, dbrc)
        return RTI_FALSE;
    ...
    while (DB_Cursor_get_next(cursor,
                              (DB_Record_T *)&port_entry) == DB_RETCODE_OK)
    {
        if (port_entry == NULL)
        {
            continue;
        }
        if (port_entry->is_open)
        {
            continue;
        }
        sock_addr.sin_family = AF_INET;
        sock_addr.sin_port = NETIO_htons(port_entry->source.port);
        sock_addr.sin_addr.s_addr = port_entry->source.value.ipv4.address;

        ret = NETIO_AutosarSocket_find_local_addr_id((struct sockaddr *)&sock_addr, &receive_local_addr_id);
        if ((receive_local_addr_id != local_addr_id) || (ret != E_OK))
        {
            continue;
        }

        /* 2a. Deregister old socket from the recvnotify callback table.
         *     pass user_data = NULL to clear the entry.
         */
        (void)netiosock_recvnotify(port_entry->_sock, NULL, NULL);

        /* 2b. Acquire a new socket ID from TcpIp. */
        new_sock = netiosock_socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
        if (new_sock == (RTI_SOCKET)-1)
        {
            UDP_LOG_SOCKET_CREATE(OSAPI_LOGKIND_ERROR, 0)
            DB_Cursor_finish(udp_intf->rx_thread_table, cursor);
            return RTI_FALSE;
        }

        /* 2c. Bind the new socket to the same local port. */
        OSAPI_Memory_zero(&sock_addr, sizeof(sock_addr));
        sock_addr.sin_family = AF_INET;
        sock_addr.sin_port =
            NETIO_htons((RTI_UINT16)port_entry->source.port);
        sock_addr.sin_addr.s_addr = port_entry->source.value.ipv4.address;

        rc = netiosock_bind(new_sock,
                            (struct sockaddr *)&sock_addr,
                            sizeof(struct sockaddr_in));
        if (rc < 0)
        {
            UDP_LOG_SOCKET_BIND(OSAPI_LOGKIND_ERROR,
                                port_entry->source.value.ipv4.address,
                                port_entry->source.port, 0)
            (void)netiosock_close(new_sock);
            DB_Cursor_finish(udp_intf->rx_thread_table, cursor);
            return RTI_FALSE;
        }

        /* 2d. Re-register the receive callback with the new socket ID. */
        if (netiosock_recvnotify(new_sock,
                                 UDP_Interface_receive_packet,
                                 port_entry) != 0)
        {
            UDP_LOG_SOCKET_BIND(OSAPI_LOGKIND_ERROR,
        ...
        /* 2e. Commit: update the port entry with the new socket ID. */
        port_entry->_sock = new_sock;
        port_entry->is_open = RTI_TRUE;
    }
 ...
 */
RTI_PRIVATE RTI_BOOL
UDP_Interface_close_socket_intf(struct UDP_Interface *udp_intf,
                                TcpIp_SocketIdType socket_id,
                                RTI_BOOL *socket_found)
{
    DB_Cursor_T cursor = NULL;
    struct UDPPortEntry *port_entry = NULL;
    DB_ReturnCode_T dbrc;

    *socket_found = RTI_FALSE;
    ...
    while (DB_Cursor_get_next(cursor,(DB_Record_T *)&port_entry)
            == DB_RETCODE_OK)
    {
        if (port_entry == NULL)
        {
        ...
        if (port_entry->_sock != socket_id)
        {
            continue;
        }
        port_entry->is_open = RTI_FALSE;
        *socket_found = RTI_TRUE;
        break;
    }
...
/*ci
 * \brief MICRO-12928: Re-open and re-bind all UDP sockets on all registered
 *        AUTOSAR UDP interfaces after a NoCom/FullCom lifecycle transition.
 *
 * Iterates over all UDP interfaces registered in the
 * UDP_Interface_fv_AutosarInstances array and calls the per-interface
 * recovery logic for each one.  Best-effort: if one interface fails,
 * recovery continues for the remaining interfaces.
 *
 * \return RTI_TRUE if all interfaces recovered successfully, RTI_FALSE if
 *         any interface failed recovery.
 */
...
UDP_Interface_recover_sockets(TcpIp_LocalAddrIdType local_addr_id)
{
    ...
    if ((UDP_Interface_fv_AutosarInstances == NULL) ||
        (UDP_Interface_fv_AutosarInstanceCount == 0))
    {
        return RTI_FALSE;
    }

    for (idx = 0; idx < UDP_Interface_fv_AutosarInstanceCount; ++idx)
    {
        if (!UDP_Interface_recover_intf(
                UDP_Interface_fv_AutosarInstances[idx], local_addr_id))
        {
            UDP_LOG_SOCKET_CLOSE(OSAPI_LOGKIND_ERROR, errno)
            return RTI_FALSE;
        }
 ...
 * \param[in] socket_id The socket ID of the socket to close.
 *
 * \return RTI_TRUE if the function succeeded, RTI_FALSE on failure. Note that if the
 *         socket was not found, the function still returns RTI_TRUE.
 */
RTI_BOOL
UDP_Interface_close_socket(TcpIp_SocketIdType socket_id)
{
    RTI_UINT32 idx;
    RTI_BOOL socket_found = RTI_FALSE;
    ...
    for (idx = 0; idx < UDP_Interface_fv_AutosarInstanceCount; ++idx)
    {
        if (!UDP_Interface_close_socket_intf(
            UDP_Interface_fv_AutosarInstances[idx], socket_id, &socket_found))
        {
            UDP_LOG_SOCKET_CLOSE(OSAPI_LOGKIND_ERROR, errno)
            return RTI_FALSE;
        }
                                      ...
                                      struct RT_ComponentListener *listener)
{
    ...
    /* Register this interface in the AUTOSAR recovery array */
    if ((UDP_Interface_fv_AutosarInstances != NULL) &&
        (UDP_Interface_fv_AutosarInstanceCount <
         UDP_INTERFACE_AUTOSAR_MAX_INSTANCES))
    {
        UDP_Interface_fv_AutosarInstances[
            UDP_Interface_fv_AutosarInstanceCount] = retval;
        ++UDP_Interface_fv_AutosarInstanceCount;
    }
    else
    {
        /* Return error as we cannot register more interfaces */
        return NULL;
    }
#endif
...
#if defined(RTI_AUTOSAR) && !defined(RTI_CERT)
    /* Deregister this interface from the AUTOSAR recovery array.
     * Swap with the last entry to keep the array compact.
     */
    if (UDP_Interface_fv_AutosarInstances != NULL)
    {
        RTI_UINT32 idx;
        for (idx = 0; idx < UDP_Interface_fv_AutosarInstanceCount; ++idx)
        {
            if (UDP_Interface_fv_AutosarInstances[idx] == self)
            {
                UDP_Interface_fv_AutosarInstances[idx] =
                    UDP_Interface_fv_AutosarInstances[
                        UDP_Interface_fv_AutosarInstanceCount - 1];
                UDP_Interface_fv_AutosarInstances[
                    UDP_Interface_fv_AutosarInstanceCount - 1] = NULL;
                --UDP_Interface_fv_AutosarInstanceCount;
                break;
            }
        }
...
#ifndef RTI_CERT
    /* Allocate the AUTOSAR interface recovery array if not yet allocated.
     * This is safe to call multiple times — only the first call allocates.
     */
    if (UDP_Interface_fv_AutosarInstances == NULL)
    {
        OSAPI_Heap_allocate_buffer(
            (char **)&UDP_Interface_fv_AutosarInstances,
            (RTI_SIZE_T)(sizeof(struct UDP_Interface *)
                         * UDP_INTERFACE_AUTOSAR_MAX_INSTANCES),
            OSAPI_ALIGNMENT_DEFAULT);
        if (UDP_Interface_fv_AutosarInstances == NULL)
        {
...
#if defined(RTI_AUTOSAR)
    /* Free the AUTOSAR interface recovery array */
    if (UDP_Interface_fv_AutosarInstances != NULL)
    {
        OSAPI_Heap_free_buffer(UDP_Interface_fv_AutosarInstances);
        UDP_Interface_fv_AutosarInstances = NULL;
        UDP_Interface_fv_AutosarInstanceCount = 0;
    }
