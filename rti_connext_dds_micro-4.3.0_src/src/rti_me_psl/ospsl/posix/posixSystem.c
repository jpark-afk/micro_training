...
#define OSAPISYSTEM_MAX_TIMERS  \
    OSAPI_SystemPosix_fv_System->_parent.property.max_timers

/*ci \brief The timer resolution in ms
 */
#define OSAPISYSTEM_TIMER_RESOLUTION_MS    10
...
struct OSAPI_SystemPosix
{
    ...
    /*ci \brief Accumulated ticks
     */
    RTI_UINT64 tick_acc;

    /*ci \brief Last tick since the timers were run
     */
    RTI_UINT64 tick_last;
};
...
/*ci \brief Return the current system tick in nanoseconds.
 *
 * \details
 * Return the current system time. If possible the monotonic clock is used,
 * otherwise fallback to the real-time clock. Since this is a compile time
 * option the same clock is always used.
 *
 * \return System time as a 64-bit unsigned integer.
 */
RTI_PRIVATE RTI_UINT64
OSAPI_PosixSystem_get_system_time(void)
{
    ...
    /* An unsigned 64-bit integer can count 213 503.982 days since boot time
     * (monotonic clock) or the epoch (real-time clock)
     */
    if ((current_time.tv_sec < 0) || (current_time.tv_nsec < 0))
    {
        return 0;
    }

    return (RTI_UINT64)current_time.tv_sec * OSAPI_TIME_NSEC_PER_SEC
            + (RTI_UINT64)current_time.tv_nsec;
}
                        ...
                        OSAPI_TimerTickHandlerFunction tick_handler)
{
    ...
    {
        if (!OSAPI_Atomic_load(
                &OSAPI_SystemPosix_fv_System->timer_handler[i].enabled,
...
OSAPI_PosixSystem_stop_timer(OSAPI_Timer_T self)
{
    ...
    for (i = 0; i < OSAPISYSTEM_MAX_TIMERS; ++i)
    {
        ...
        {
            OSAPI_Atomic_store(
                    &OSAPI_SystemPosix_fv_System->timer_handler[i].deleted,
                    RTI_TRUE,
                    OSAPI_ATOMIC_MEMORY_ORDER_RELEASE);

            /* Wait for the tick routine to acknowledge the deletion */
            while (OSAPI_Atomic_load(
                    &OSAPI_SystemPosix_fv_System->timer_handler[i].deleted,
                    OSAPI_ATOMIC_MEMORY_ORDER_ACQUIRE));
...
OSAPI_PosixSystem_get_timer_resolution(void)
{
    ...
    return OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_TIME_NSEC_PER_MSEC;
...
/*ci
 * \brief Run all the system timers one tick
 *
 * \details
 *
 ...
 */
RTI_PRIVATE void
OSAPI_SystemPosix_run_timers_one_tick(struct OSAPI_SystemPosix *self)
{
    RTI_INT32 i;
    ...
    for (i = 0; (i < OSAPISYSTEM_MAX_TIMERS); ++i)
    {
         ...
         * at most one tick to delete a timer.  Use an acquire load so that
         * the handler and param reads below cannot be reordered before this
         * check.
         */
        if (OSAPI_Atomic_load(&self->timer_handler[i].enabled,
                              OSAPI_ATOMIC_MEMORY_ORDER_ACQUIRE))
        {
        ...
        if (OSAPI_Atomic_load(&self->timer_handler[i].deleted,
                              OSAPI_ATOMIC_MEMORY_ORDER_ACQUIRE))
        {
            OSAPI_Atomic_store(&self->timer_handler[i].deleted,
                               RTI_FALSE,
                               OSAPI_ATOMIC_MEMORY_ORDER_RELEASE);

            OSAPI_Atomic_store(&self->timer_handler[i].enabled,
                               RTI_FALSE,
                               OSAPI_ATOMIC_MEMORY_ORDER_RELEASE);
...
/*ci
 * \brief Run all the system timers and take into account accumulated time
 *
 * \param[in] self Pointer to POSIX system instance
 */
RTI_PRIVATE void
...
{
    RTI_UINT64 tick_after;

    tick_after = OSAPI_PosixSystem_get_system_time();
    if (tick_after < self->tick_last)
    {
        return;
    }

    self->tick_acc += tick_after - self->tick_last;
    self->tick_last = tick_after;

    while (self->tick_acc >= (RTI_UINT64)self->tick_resolution_ns)
    {
        self->tick_acc -= (RTI_UINT64)self->tick_resolution_ns;
        OSAPI_SystemPosix_run_timers_one_tick(self);
    }
}
...
OSAPI_SystemPosix_timer_thread(struct OSAPI_ThreadInfo* thread_param)
{
    ...
    self->tick_last = OSAPI_PosixSystem_get_system_time();
...
OSAPI_SystemPosix_start_tick(struct OSAPI_SystemPosix *self)
{
    ...
    if (self->timer_resolution.tv_nsec > (OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_TIME_NSEC_PER_MSEC))
    {
    ...
    {
        self->tick_resolution.it_interval.tv_nsec =
            ((OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_TIME_NSEC_PER_MSEC / self->timer_resolution.tv_nsec)
                * self->timer_resolution.tv_nsec);
    }
...
#else
    self->tick_resolution_ns = (RTI_INT32)OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_TIME_NSEC_PER_MSEC;
#endif /* !RTI_VXWORKS */
#endif
    ...
    self->tick_resolution_ns = OSAPISYSTEM_TIMER_RESOLUTION_MS * OSAPI_TIME_NSEC_PER_MSEC;
...
OSAPI_SystemPosix_initialize_impl(struct OSAPI_SystemPosix *self)
{
    ...
    for (i = 0; i < OSAPISYSTEM_MAX_TIMERS; ++i)
    {
        ...
        OSAPI_Atomic_initialize(&self->timer_handler[i].enabled, RTI_FALSE);
#ifndef RTI_CERT
        OSAPI_Atomic_initialize(&self->timer_handler[i].deleted, RTI_FALSE);
#endif
    }
