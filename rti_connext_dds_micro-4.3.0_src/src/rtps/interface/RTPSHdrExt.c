 ...
 *                       header extensions
 *
 * Copyright (c) 2020-2026 Real-Time Innovations, Inc. All rights reserved.
 *
...
/*ci \brief Maximum number of scatter-gather buffers for checksum
 *          calculation. Same value as in the UDP interface.
 */
                                 ...
                                 RTI_BOOL byte_swap)
{
    ...
    RTPS_Checksum_T saved_checksum = RTPS_CHECKSUM_INIT_ZERO;
    RTPS_Checksum_T calculated_checksum = RTPS_CHECKSUM_INIT_ZERO;
    char *ptr = NULL;
 ...
 *
 * \details This function can add space for the following reasons:
 *   1. The header extension is required for the checksum OR
 *   2. The header extension is required for the AAD
 *
 *   When  both AAD and checksum are enabled, the header extension will not add the Legacy CRC32
 *   This is confirmed when RTPS is initialized.
 * \param[in] intf - The RTPS interface
                                    ...
                                    struct RTPS_HEADER_EXT **hdrext)
{
    ...
    *hdrext = NULL;
    ...
    if ((intf->checksum_index == RTPS_NO_CHECKSUM_INDEX)
        && !RTPS_Interface_is_aad_enabled(intf))
    {
        return RTI_TRUE;
    }

    /* if we are here the header needs to be added. Lets check if space for checksum is required. */
    if ((intf->checksum_index > RTPS_NO_CHECKSUM_INDEX))
    {
    ...
    if (!NETIO_PacketBuffer_adjust_head(pbuf, 0 - (RTI_INT32)(hdr_size)))
    {
        RTPS_LOG_NETIO_PACKET_SET_HEAD(OSAPI_LOGKIND_ERROR)
        return RTI_FALSE;
    }

    *hdrext = NETIO_PacketBuffer_get_head(pbuf);

    OSAPI_Memory_zero(*hdrext, hdr_size);
...
void
RTPS_Interface_set_header_extension_header(struct RTPS_Interface *intf,
    struct RTPS_HEADER_EXT *hdrext)
{
    RTI_UINT8 flags = 0;
                                    ...
                                    NETIO_Packet_T *packet)
{
    ...
    struct REDA_Buffer buf[MAX_SCATTER_GATHER_BUFS];
    RTPS_Checksum_T checksum = RTPS_CHECKSUM_INIT_ZERO;
    char *msg_ptr = (char *)hdrext;
    RTI_UINT32 buf_len = 0;
    struct NETIO_PacketBuffer *pbuf;
    ...
    {
        /* Since no checksum is needed, we are done */
        return RTI_TRUE;
    }
    ...
    if (pbuf != NULL)
    {
        while (buf_len < MAX_SCATTER_GATHER_BUFS)
        {
            REDA_Buffer_set(&buf[buf_len],
                            NETIO_PacketBuffer_get_head(pbuf),
                            NETIO_PacketBuffer_get_length(pbuf));
            buf_len++;

            pbuf = NETIO_PacketBuffer_get_next(pbuf);
            if (pbuf == NULL)
            {
                break;
            }
        ...
        if (pbuf != NULL)
        {
            RTPS_LOG_CHECKSUM_TOO_MANY_SG_BUFFERS(OSAPI_LOGKIND_ERROR)
            return RTI_FALSE;
        }
    }
    else
    {
        REDA_Buffer_set(&buf[0],
    ...
    if (!RTPS_Interface_checksum_calculate(intf,intf->checksum_index,buf,buf_len,&checksum))
    {
                                 ...
                                 RTI_SIZE_T *msg_length)
{
    ...
    *msg_length = 0;
    ...
    /* Check if the L-bit is set, if so the messageLength field is present
     * and must be deserialized.
     */
    if (sub_msg->hdr.flags & RTPS_HEADER_EXTFLAG_L)
    {
        ...
        *msg_length = hdrext_msglen;
        ...
        length_size = RTI_SIZEOF(CDR_UnsignedLong);
        skip_bytes += length_size;
    }
    else
    {
        /* If the messageLength is not present, then we must assume that the
         * message is the entire remaining payload. This assumption is fine
         * as long as a checksum is not present or this payload contains only
         * one RTPS message. If this message contains a checksum and the
         * payload contains more than one RTPS message, then message may be
         * dropped because the checksum is calculated incorrectly. This
         * behavior is consistent with Connext Pro.
         */
        hdrext_msglen = payload_length;
    }
    ...
    if (c_bits != 0)
    {
         ...
         * is still just after the submessage header. Subtract the size of the
         * messageLength field if present because msg_ptr has already been
         * incremented when it was deserialized if the L-bit is set.
         */
        msg_ptr += skip_bytes - length_size;
    }
    ...
    if (c_bits == 0)
    {
        if (RTPS_Interface_is_require_crc(intf))
        {
    ...
    if (RTPS_Interface_is_check_crc(intf))
    {
        chksum_ptr = OSAPI_Compiler_reinterpret_cast(RTPS_Checksum_T*,msg_ptr);
                              ...
                              RTI_SIZE_T *msg_length)
{
    ...
    *msg_length = 0;
    ...
    sub_msg = OSAPI_Compiler_reinterpret_cast(union RTPS_MESSAGES*,msg_ptr);
    ...
    else if (RTPS_Interface_is_require_crc(intf))
    {
        drop_message = RTI_TRUE;
    }
    else
