...
/*
 * \brief
 * Return number of bytes to serialize for lease duration
 *
 * \param[in] size Current size of serialized stream
 *
 * \return Number of additional bytes to serialize
 */
RTI_UINT32
...
/*ci
 * \brief
 * Serialize two octets
 *
 * \param[in] stream Serialization stream
 * \param[in] in Octet pair to serialize
 * \param[in] param Unused
 *
 * \return RTI_TRUE on success, RTI_FALSE on failure
 */
                        ...
                        void *param)
{
    ...
    if (!CDR_Stream_check_size(stream, 2 * CDR_OCTET_SIZE))
    {
        return RTI_FALSE;
    }

    if (!CDR_Stream_serialize_octet(stream, &in[0]))
    {
        return RTI_FALSE;
    }

    if (!CDR_Stream_serialize_octet(stream, &in[1]))
    {
        return RTI_FALSE;
    }
...
/*ci
 * \brief
 * Deserialize two octets
 *
 * \param[in] stream Deserialization stream
 * \param[out] out Deserialized octets
 * \param[in] param Unused
 *
 * \return RTI_TRUE on success, RTI_FALSE on failure
 */
                          ...
                          void *param)
{
    ...
    if (!CDR_Stream_deserialize_octet(stream, &out[0]))
    {
        return RTI_FALSE;
    }

    if (!CDR_Stream_deserialize_octet(stream, &out[1]))
    {
        return RTI_FALSE;
    }
                            ...
                            void *param)
{
    ...
    {
        *((RTI_UINT8*)(stream->buff_ptr++)) = loc->address[i];
    }
                              ...
                              void *param)
{
    ...
    else
    {
        ...
        OSAPI_Memory_copy(&loc->address[0], stream->buff_ptr, 4);
        stream->buff_ptr += 4;
    }
